...
if (JSON.stringify(first) !== JSON.stringify(second)) {
  ...
  const changed = second.files
    ...
    .map((file) => file.path);
  throw new Error(
    `CEM Site clean builds are not deterministic: ${changed.join(', ') || 'aggregate drift'}`,
  );
}
...
console.log(
  `CEM Site determinism verified (${report.fileCount} files, sha256:${report.aggregateSha256})`,
);

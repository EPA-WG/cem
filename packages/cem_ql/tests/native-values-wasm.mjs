...
const source = `{child | {attribute @name=count @type=integer @value=002}{attribute @name=day @type=date @value=2024-02-29}{attribute @name=label @type=node @content-type=text/html @value='{data:read("<name>ivy<em>saur</em></name>", "xml").root.children}'}{attribute @name=empty @type=any @value='{()}'}}`;
function produce() {
...
{body | {xpath @context=node @sequence-type="xs:string" | {expression | string(.)}}}}}`, 'memory:native-value-functions.cemt'));
    ...
    const template = JSON.parse(wasm.compileTemplate('{attribute @name=count @type=integer @minInclusive=1 @required=true}{attribute @name=day @type=date @required=true}{attribute @name=label @type=node @required=true}{attribute @name=empty @type=any}{output | {$count + 1}{span | {$day}}{$label}{b | {$native:call("value.text", label)}}{i | {$seq:count(empty)}}}', JSON.stringify(bindings.map(b => b.name))));
    try {

...
use cem_ql::{
    ...
    render::{
        compile_template_module_closure, render_compiled_template, CompileTemplateOptions,
        RenderPlan, RenderPlanNode, TemplateData, TemplateModuleClosure, TemplateModuleSource,
    },
};
...
const LIST: &str = r#"<catalog><pokemon><name pokemon-id="3">venusaur</name><type>grass</type></pokemon><pokemon><name pokemon-id="2">ivysaur</name><type>grass</type></pokemon></catalog>"#;
...
fn render(template: &str, source: &str, mut slices: Vec<(&str, Item)>) -> RenderPlan {
    let hash = |text: &str| {
...
fn source_identity_and_name_sorting_survive_cell_overrides() {
    ...
    let button = find(&initial.nodes, "button")
        .into_iter()
...
fn nested_collections_keep_base_structure_and_apply_the_selected_rule() {
    let source = LIST.replace("<type>grass</type>",
        "<related><entry><name pokemon-id='2'>ivysaur</name></entry><entry><name pokemon-id='3'>venusaur</name></entry></related>");
    let plan = render(pokemon_template(), &source, vec![]);

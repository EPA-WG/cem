...
fn render_template_uses_explicit_structured_data_document() {
    ...
    let rendered = render_template(
        "{span | {$ datadom.attributes.label}-{$ datadom.dataset.variant}-{$ datadom.slices.open}-{$ datadom.payload.text}-{$datadom.slots.leading.text}}",
        &data,
    );

    assert_eq!(
        rendered.rendered,
        "<span>Email-compact-true-Payload-Lead</span>"
    );

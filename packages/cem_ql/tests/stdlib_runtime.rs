...
fn tier_a_registry_lists_every_documented_module_function() {
    ...
    for (module, name, arity) in [
        ...
        ("cem:stdlib/dom", "reference", 1), ("cem:stdlib/dom", "clone", 1),
        ("cem:stdlib/dom", "element", 1), ("cem:stdlib/cemt", "apply_templates", 2),
    ] {
        assert!(registry.resolve(module, name, arity).is_some(), "{module}:{name}/{arity}");
    }
    assert!(registry
    ...
    assert!(registry
        ...
        .is_some());
    for name in ["can_parse", "parse", "href", "assemble", "with_parts"] {
        assert!(registry.resolve("cem:stdlib/url", name, 2).is_some());
        assert_eq!(registry.resolve("cem:stdlib/url", name, 1).is_some(), name != "with_parts");
        assert!(registry.resolve("cem:stdlib/url", name, 0).is_none());
        assert!(registry.resolve("cem:stdlib/url", name, 3).is_none());
    }

...
.primary-action {
  ...
  transition:
    background-color var(--cem-duration-noticeable) var(--cem-easing-smooth),
...
CEM is expected to survive light, dark, contrast, native/system, forced-colors, responsive, and dense UI contexts without
changing the semantic meaning of product code.

## Relation to Other UX Theming

CEM is not an in-place replacement for Material Design 3, Angular Material, MUI, Tailwind, or a brand-token system. It is

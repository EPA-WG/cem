...
| 13 | `if` conditional construct                              | `badge.html:192`, `input.html:207`                                              | ❌     | Renders as a literal inert `<if>` element; children always emitted. Needs bridge/cem-ml lowering.               |
| 14 | `choose`/`when`/`otherwise` conditional                 | `icon.html:79`, `icon-link.html:91`                                              | ❌     | Renders as literal inert elements; no branch selection.                                                          |
...
| 18 | Declarative `<slot>` / named slots                      | `icon.html:85`, `input.html:206`, `autocomplete.html:85`                         | ❌     | Rendered as a literal `<slot>`; instance payload is not projected into it. No slot projection in light DOM.      |
...
| 21 | `<data>` / `<option>` instance payloads                 | `autocomplete.html:112`, `input.html:275`                                        | ❌     | Captured inert into the data island; XPath `//data`/`//option` consumption unimplemented.                       |

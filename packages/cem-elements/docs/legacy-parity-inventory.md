...
| `attribute select="..."` derives an exposed attribute from slice/data state | `docs/attributes.md`; `demo/attributes.html` | Supported | Canonical `attribute @select` evaluates CEM-QL over `datadom.*`; selected values update `$name` and `datadom.attributes.name` in the current render before loop-safe host reflection |
...
| Slice updates from DOM events rerender output | README interactivity; `demo/data-slices.html` | Supported | `LegacySliceInputEventParity`, `SliceEventInvalidationRerenders`, `SliceEventExpressionParity`, and all 16 legend-scoped `data-slices.html` source contracts |
| Default and multiple events, multiple slice/attribute targets, synthetic slice directives, pointer payloads, and checkbox/radio coercion | `demo/data-slices.html` cases A, B, 1-13 | Supported | Form controls default to `change` and control-aware values; `/datadom/attributes/*` targets reflect to the host; nested `slice` directives initialize from `value` and bind independent events to their parent control; serialized mouse coordinates and checkbox/radio value semantics are covered by `LegacyDataSliceControlParity` and the complete demo fixture |

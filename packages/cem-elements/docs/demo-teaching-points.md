...
| `cell-overrides.html` | An inline template matches `name` cells and renders a Pokémon image plus name. A separate module matches zero stock within product rows and renders a warning. Both import the unchanged viewer; unmatched fields, sorting and source-loaded image resolution remain visible. |

...
| `dom-merge.html` | Textarea commits on change and opts into layout whitespace handling; typed whitespace stays exact. Input edits retain control identity, focus and selection. Empty/whitespace input and Unicode codepoint counts are checked for CEM-QL and XPath. |

...
async function verifyFormAssociatedDce(sample: HTMLElement): Promise<void> {
    ...
    expect(document.activeElement).toBe(requiredElement(choices[0], 'button[aria-label="Choose fruit"]'));
    await state('', '');
    await choose(0, 'Apple');
    await state('🍏', '');
    await userEvent.click(requiredElement(sample, 'button[type="submit"]'));
    expect(document.activeElement).toBe(requiredElement(choices[1], 'button[aria-label="Choose fruit"]'));
    await state('🍏', '');

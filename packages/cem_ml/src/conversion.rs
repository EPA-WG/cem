...
use crate::transform_template::{
    ...
    TransformTemplateEncodedArtifactMode, TransformTemplateEncodedArtifactPayload,
...
const HTML_FORMAT_CEMT_STAGE_SPEC: CemTreeCemtOutputStageSpec = CemTreeCemtOutputStageSpec {
    adapter_id: "html-format-cemt",
    artifact_kind: CEM_TREE_FORMATTER_ARTIFACT_KIND,
    declaration_element: "{format-function",
    function_kind: TransformTemplateOutputFunctionKind::Format,
    function_name: "html.format-document",
    role: "formatter",
};

const HTML_COLOR_CEMT_STAGE_SPEC: CemTreeCemtOutputStageSpec = CemTreeCemtOutputStageSpec {
    adapter_id: "html-color-cemt",
    artifact_kind: CEM_TREE_COLORIZER_ARTIFACT_KIND,
    declaration_element: "{color-function",
    function_kind: TransformTemplateOutputFunctionKind::Color,
...
fn execute_conversion_cem_tree_output_stage_body(
    ...
    let reject_encode_evaluation = |body_binding: &TransformTemplateEncodeBinding,
                                    _subject: &Value| {
        Err(format!(
            "CEMT {} `{}` requires a direct CEMT body; encode(...) facade attempted to dispatch `{}`",
            stage.role, stage.function_name, body_binding.function.name
...
fn execute_conversion_output_pipeline_with_environment_subject(
    ...
    if let Some(formatted_cemt_tree) = formatted_cemt_tree {
...
pub fn execute_csv_document_output_pipeline_with_environment(
    ...
    let (writer_artifact, color_execution, colored_cem_tree) = if wants_color {
        ...
        let colored_artifact = color_binding.artifact_from_cemt_value(colored_output);
...
impl XmlDocumentOutputSubject for HtmlDocumentAst {
    fn source_line_ending(&self) -> Option<&str> {
        self.line_ending.as_deref()
    }
...
pub fn execute_html_document_output_pipeline_with_environment(
    environment: &ConversionOutputPipelineEnvironment<'_>,
...
impl XmlFamilyOutputSpec {
    ...
    const fn html() -> Self {
        Self {
            label: "HTML",
            formatter: HTML_FORMAT_CEMT_STAGE_SPEC,
            colorizer: HTML_COLOR_CEMT_STAGE_SPEC,
            content_type: HTML_CONTENT_TYPE,
            schema: HTML_SCHEMA_URI,
            category: "html-document",
            subject_kind: "html-document",
            formatter_option_prefix: "html.",
            converter_id: "html-direct-output",
            diagnostic_node: "html",
        }
    }
...
fn conversion_output_pipeline_formatted_cem_tree_artifact(
    ...
    TransformTemplateEncodedArtifact {
        ...
        value: TransformTemplateEncodedArtifactPayload::CemtRuntime(
...
fn execute_conversion_output_pipeline_from_typed_formatted_artifact(
    ...
    let functions = conversion_cem_tree_output_function_registry(environment, pipeline);
    let implementations = TransformTemplateEncodeImplementationRegistry::with_builtin_encoders();
    let (writer_binding, writer_cemt_tree, color_execution, color_elapsed_ns, colored_cemt_tree) =
        if conversion_output_pipeline_should_skip_color_stage(pipeline) {
            ...
            let (_, colored_cemt_tree, color_execution) = match color_result {
                Ok(output) => output,
                Err(message) => {
                    diagnostics.push(output_pipeline_diagnostic(
                        converter_id,
            ...
            let Some(colored_cemt_tree) = colored_cemt_tree else {
                diagnostics.push(output_pipeline_diagnostic(
                    converter_id,
                    diagnostic_node,
                    diagnostic_uri,
                    format!(
                        "CEMT colorizer `{}` did not return a typed colored artifact",
                        color_binding.function.name
                    ),
                ));
                return ConversionOutputPipelineExecution {
                    ...
                    diagnostics,
                    format_execution,
                    color_execution: Some(color_execution),
                    format_elapsed_ns,
                    color_elapsed_ns,
                    formatted_cemt_tree: Some(formatted_cemt_tree),
                    ..ConversionOutputPipelineExecution::default()
            ...
            (
                ...
                Some(color_execution),
                color_elapsed_ns,
                Some(colored_cemt_tree),
            )
        };

    let writer_started = Instant::now();
    let writer_result = execute_transform_template_typed_cem_tree_writer(
        TransformTemplateTypedCemTreeWriterRequest {
            binding: &writer_binding,
            artifact: writer_cemt_tree,
            insertion_context: &pipeline.writer_insertion_context,
        },
    );
    let writer_elapsed_ns = Some(writer_started.elapsed().as_nanos());
    let artifact = match writer_result {
        ...
        Err(message) => {
            diagnostics.push(Diagnostic {
                ...
                message: format!(
                    "encoded artifact produced `cem-tree` cannot use the typed writer adapter: {message}"
                ),
    ...
    ConversionOutputPipelineExecution {
        output: Some(
            artifact
                .value
                .into_public_value()
                .expect("typed CEM tree writer projects to the public response"),
        ),
        raw_cem_tree: None,
        formatted_cemt_tree: Some(formatted_cemt_tree),
        colored_cemt_tree,
        ...
        color_elapsed_ns,
        writer_elapsed_ns,
        formatted_cem_tree: None,
        colored_cem_tree: None,
        diagnostics,
    }
}
  ...
  <p class="lead" id="x">ok</p>
    ...
    fn native_cem_tree_pipeline_lowers_typed_per_node_operations() {
        ...
        let overlay = formatted
            .formatted_overlay()
            .expect("formatted artifact has an overlay");
        assert!(overlay.retains_node(&root_path));
        assert!(overlay.retains_node(&title_path));
        assert!(!overlay.retains_node(&skipped_whitespace_path));
        assert!(overlay.retains_node(&body_path));
        assert!(overlay.retains_node(&title_text_path));
        assert_eq!(
            overlay
                .node_operations
                .iter()
    ...
    fn native_typed_cem_tree_writer_matches_runtime_profiles_and_preserves_maps() {
        let source_map = SourceMapStack {
            frames: vec![crate::source_map::SourceMapFrame {
                ...
                span: crate::source_map::FrameSpan::Single(crate::source::ByteRange::new(3, 21)),
        ...
        let compact = direct_cem_output_pipeline();
        let mut pretty = direct_cem_output_pipeline();
        pretty.cemt_options.formatter_profile = Some("pretty".to_owned());
        pretty.cemt_insertion_context.formatter_profile = Some("pretty".to_owned());
        pretty.writer_insertion_context.formatter_profile = Some("pretty".to_owned());
        let mut tabular = direct_cem_output_pipeline();
        tabular.cemt_options.formatter_profile = Some("tabular".to_owned());
        tabular.cemt_insertion_context.formatter_profile = Some("tabular".to_owned());
        tabular.writer_insertion_context.formatter_profile = Some("tabular".to_owned());
        let mut terminal = direct_cem_output_pipeline();
        terminal.cemt_options.color_profile = Some("terminal".to_owned());
        terminal.cemt_insertion_context.color_profile = Some("terminal".to_owned());
        terminal.writer_insertion_context.color_profile = Some("terminal".to_owned());
        terminal.writer_insertion_context.output_color_type = Some("ansi-256".to_owned());
        let html = direct_html_output_pipeline();
        let markdown = direct_markup_output_pipeline(
        ...
        for (profile, pipeline) in [
            ...
            assert!(typed.formatted_cem_tree.is_none(), "{profile}");
            assert!(typed.colored_cem_tree.is_none(), "{profile}");
            assert_eq!(typed.source_map.as_ref(), Some(&source_map), "{profile}");
            assert!(!typed.output_spans.is_empty(), "{profile} output spans");
            assert!(typed.output_spans.iter().all(|span| {
                span.origin
                    .frames
                    .iter()
    ...
    fn native_cem_tree_formatter_ingress_has_no_encoded_json_boundary() {
        ...
        for forbidden in [
            ...
            assert!(
                !native_pipeline.contains(forbidden),
                "native CEMT writer pipeline must not use `{forbidden}`"
            );
        }
...
"#
        ...
        binding
            .artifact_from_cemt_value(formatted)
            .validate_insertion(
                &TransformTemplateEncodedArtifactInsertionContext::from_encoding_target(
                    &request.target,
                    Some(TransformTemplateOutputProducedKind::CemTree),
...
"#;
    ...
    #[test]
    fn builtin_html_lifecycle_output_pipeline_executes_package_cemt_assets() {
        let schema_registry = SchemaRegistry::with_builtin_schemas();
        let conversion_registry = ConversionRegistry::with_builtin_converters();
        let environment = ConversionOutputPipelineEnvironment {
            schema_registry: &schema_registry,
        ...
        let source = b"<!doctype html>\n<HTML lang=en><head><title>a < b</title></head><body><svg viewBox=\"0 0 1 1\"><path></path></svg><br></body></HTML>\n";
        let (document, diagnostics) = crate::validation::html::html_document_ast_from_source_bytes(
            crate::validation::html::HtmlSourceValidationRequest {
                bytes: source,
                source_uri: "builtin:html-output",
        ...
        for (profile, layout) in [
            ...
            let execution = execute_html_document_output_pipeline_with_environment(
                &environment,
                document.clone().expect("HTML document"),
                &target_scope,
                Some("builtin:html-output"),
            );
            assert!(
                execution.diagnostics.is_empty(),
                "{profile}: {:?}",
            ...
            assert!(
                matches!(
                    ...
                    Some(ConversionOutputPipelineStageExecution::CemtAdapter {
                        ...
                        ref body_function_name,
                        ..
                    }) if adapter_id == "html-format-cemt"
                        && function_name == "html.format-document"
                        && body_function_name.as_deref() == Some("html.format-document")
                ),
                "{profile}: {:?}",
                execution.format_execution
            );
            ...
            let formatted = execution
                ...
                .as_ref()
                .unwrap_or_else(|| panic!("{profile} formatted tree"));
            assert_eq!(formatted.value["contentType"], HTML_CONTENT_TYPE);
            assert_eq!(formatted.value["schema"], HTML_SCHEMA_URI);
            assert_eq!(formatted.value["category"], "html-document");
            assert_eq!(formatted.value["formatterProfile"], profile);
            assert_eq!(formatted.value["formatNodes"][1]["value"]["layout"], layout);
            assert!(formatted.value["nodes"]
                .as_array()
                .is_some_and(|nodes| nodes.iter().any(|node| {
                    node["kind"] == "html.rcdata"
                        && node["text"] == "a < b"
                        && node["sourceMap"] != Value::Null
                })));
        }
    ...
    #[test]
    fn builtin_html_colorizer_profiles_execute_package_cemt_assets() {
        let schema_registry = SchemaRegistry::with_builtin_schemas();
        let conversion_registry = ConversionRegistry::with_builtin_converters();
        let environment = ConversionOutputPipelineEnvironment {
            schema_registry: &schema_registry,
        ...
        let (document, diagnostics) = crate::validation::html::html_document_ast_from_source_bytes(
            crate::validation::html::HtmlSourceValidationRequest {
                bytes: source,
                source_uri: "builtin:html-color-output",
                content_type: Some(HTML_CONTENT_TYPE),
            },
        );
        assert!(diagnostics.is_empty(), "{diagnostics:?}");
        ...
        for (profile, style_key, style_value) in [
            ...
            let execution = execute_html_document_output_pipeline_with_environment(
                ...
                document.clone().expect("HTML document"),
                &target_scope,
                Some("builtin:html-color-output"),
            );
            assert!(
                execution.diagnostics.is_empty(),
                "{profile}: {:?}",
...
"#;
    ...
    fn builtin_csv_colorizer_profiles_execute_package_cemt_assets() {
        for (profile, expected_path, expected_output, expected_style_key, expected_style_value) in [
            ...
            context.source_map_policy = Some(TransformTemplateSourceMapPolicy::Generated);
            binding
                .artifact_from_cemt_value(colored)
                .validate_insertion(&context)

...
const TRANSFORM_TEMPLATE_STANDARD_OUTPUT_FUNCTIONS:
    ...
    TransformTemplateStandardOutputFunctionContract {
        kind: TransformTemplateOutputFunctionKind::Color,
        name: "html.css-custom-properties",
        category: "html-color",
        subject: Some("tokens"),
        produces: TransformTemplateOutputProducedKind::Text,
        content_type: HTML_CONTENT_TYPE,
    ...
    TransformTemplateStandardOutputFunctionContract {
        kind: TransformTemplateOutputFunctionKind::Color,
        name: "terminal.ansi16",
        category: "terminal-color",
        subject: Some("tokens"),
        produces: TransformTemplateOutputProducedKind::Text,
        content_type: "text/plain",
        schema: TRANSFORM_TEMPLATE_TERMINAL_TEXT_SCHEMA_URI,
        profile: Some("ansi-16"),
    },
    TransformTemplateStandardOutputFunctionContract {
        kind: TransformTemplateOutputFunctionKind::Color,
        name: "terminal.ansi256",
        category: "terminal-color",
        subject: Some("tokens"),
        produces: TransformTemplateOutputProducedKind::Text,
        content_type: "text/plain",
        schema: TRANSFORM_TEMPLATE_TERMINAL_TEXT_SCHEMA_URI,
        profile: Some("ansi-256"),
    },
    TransformTemplateStandardOutputFunctionContract {
        kind: TransformTemplateOutputFunctionKind::Color,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct TransformTemplateOutputColorSelection {
    pub output_color_type: String,
    pub target: TransformTemplateEncodingTarget,
    pub colorizer: String,
    pub color_profile: String,
}

impl TransformTemplateOutputColorSelection {
    pub fn format_identity(&self) -> FormatIdentity {
        self.target.format_identity()
    }

    pub fn apply_to_request(&self, request: &mut TransformTemplateEncodeBindingRequest) {
        request.target = self.target.clone();
        request.options.colorizer = Some(self.colorizer.clone());
        request.options.color_profile = Some(self.color_profile.clone());
    }
}
...
pub fn parse_transform_template_output_color_type(
    ...
    let selector = selector.trim();
    if selector.is_empty() {
        return Err("output color type is empty".to_owned());
    }
    let normalized = selector.trim().to_ascii_lowercase().replace('_', "-");
    match normalized.as_str() {
        "none" | "plain" | "terminal-none" | "terminal.none" => Ok(
            transform_template_terminal_output_color_selection("none", "terminal.none"),
        ),
        "terminal" | "terminal-auto" | "terminal.auto" | "auto" | "ansi-auto" => Ok(
            transform_template_terminal_output_color_selection("auto", "terminal.auto"),
        ),
        "ansi16" | "ansi-16" | "terminal-ansi16" | "terminal-ansi-16"
        | "terminal.ansi16" => Ok(transform_template_terminal_output_color_selection(
            "ansi-16",
            "terminal.ansi16",
        )),
        "ansi256" | "ansi-256" | "terminal-ansi256" | "terminal-ansi-256"
        | "terminal.ansi256" => Ok(transform_template_terminal_output_color_selection(
            "ansi-256",
            "terminal.ansi256",
        )),
        "truecolor" | "terminal-truecolor" | "terminal.truecolor" => Ok(
            transform_template_terminal_output_color_selection("truecolor", "terminal.truecolor"),
        ),
        "html" | "html-css-vars" | "html.css-vars" | "css-vars"
        | "css-custom-properties" | "html-css-custom-properties"
        | "html.css-custom-properties" => Ok(transform_template_html_output_color_selection(
            "css-custom-properties",
            "html.css-vars",
        )),
        "html-classes" | "html.classes" | "classes" | "class-based" => Ok(
            transform_template_html_output_color_selection("classes", "html.classes"),
        ),
        "html-inline-style" | "html.inline-style" | "inline-style" => Ok(
            transform_template_html_output_color_selection("inline-style", "html.inline-style"),
        ),
        _ => Err(format!(
            "unsupported output color type `{selector}`; supported values include terminal, ansi-16, ansi-256, truecolor, auto, none, html, html-classes, html-inline-style, and html-css-vars"
        )),
    }
}

fn transform_template_terminal_output_color_selection(
    color_profile: &str,
    colorizer: &str,
) -> TransformTemplateOutputColorSelection {
    TransformTemplateOutputColorSelection {
        output_color_type: color_profile.to_owned(),
        target: TransformTemplateEncodingTarget::new(
            "text/plain",
            TRANSFORM_TEMPLATE_TERMINAL_TEXT_SCHEMA_URI,
            "terminal-color",
        ),
        colorizer: colorizer.to_owned(),
        color_profile: color_profile.to_owned(),
    }
}

fn transform_template_html_output_color_selection(
    color_profile: &str,
    colorizer: &str,
) -> TransformTemplateOutputColorSelection {
    TransformTemplateOutputColorSelection {
...
fn transform_template_encode_options_request_color(
    options: &TransformTemplateEncodeOptions,
) -> bool {
    transform_template_trimmed_selector(options.colorizer.as_deref()).is_some()
        || transform_template_trimmed_selector(options.color_profile.as_deref()).is_some()
}
...
fn transform_template_target_is_explicit_color_output(
    target: &TransformTemplateEncodingTarget,
) -> bool {
    let category = target.category.trim();
    let schema = target.schema.trim();
...
impl TransformTemplateColorBinding {
    ...
    pub fn into_encode_binding(self) -> TransformTemplateEncodeBinding {
...
impl TransformTemplateEncodeImplementationRegistry {
    ...
    pub fn with_builtin_encoders() -> Self {
        ...
        registry.register("html.classes", builtin_html_colorizer);
        registry.register("html.inline-style", builtin_html_colorizer);
        registry.register("html.css-vars", builtin_html_colorizer);
        registry.register("html.css-custom-properties", builtin_html_colorizer);
        ...
        registry.register("terminal.none", builtin_terminal_colorizer);
        registry.register("terminal.ansi16", builtin_terminal_colorizer);
        registry.register("terminal.ansi256", builtin_terminal_colorizer);
        registry.register("terminal.truecolor", builtin_terminal_colorizer);
        registry.register("terminal.auto", builtin_terminal_colorizer);
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TransformTemplateColorToken {
    role: String,
    text: String,
    href: Option<String>,
}
...
fn builtin_terminal_colorizer(
    ...
    if profile.output != TransformTemplateColorOutputKind::Terminal {
        return Err(format!(
            "terminal colorizer `{}` expected terminal color profile, got `{:?}`",
            binding.function.name, profile.output
        ));
    }

    let tokens = transform_template_color_tokens_from_subject(subject)?;
    let mut output = String::new();
    for token in tokens {
        output.push_str(&transform_template_render_terminal_color_token(
            &token, &profile,
        ));
    }
    if !output.is_empty()
...
fn builtin_html_colorizer(
    ...
    if profile.output != TransformTemplateColorOutputKind::Html {
        return Err(format!(
            "HTML colorizer `{}` expected HTML color profile, got `{:?}`",
            binding.function.name, profile.output
        ));
    }
...
fn validate_builtin_colorizer_binding(
    ...
    if binding.function.kind != TransformTemplateOutputFunctionKind::Color {
        return Err(format!(
            "colorizer implementation `{}` cannot execute non-color function `{:?}`",
            binding.function.name, binding.function.kind
        ));
    }
    if !binding.function.name.starts_with(expected_name_prefix) {
        return Err(format!(
            "colorizer implementation `{expected_name_prefix}*` cannot execute `{}`",
            binding.function.name
        ));
    }
    let category = binding.identity.target.category.trim();
    if category != expected_category && !category.starts_with(&format!("{expected_category}-")) {
        return Err(format!(
            "colorizer `{}` expected category `{expected_category}`, got `{}`",
            binding.function.name, binding.identity.target.category
        ));
    }
    if binding.identity.produces != TransformTemplateOutputProducedKind::Text {
        return Err(format!(
            "colorizer `{}` expected text output, got `{:?}`",
            binding.function.name, binding.identity.produces
        ));
    }
    Ok(())
}

fn transform_template_binding_color_profile(
    binding: &TransformTemplateEncodeBinding,
) -> Result<TransformTemplateColorOutputProfile, String> {
    transform_template_color_output_profile_for_target(
        ...
        binding
            ...
            .color_profile
            .as_deref()
            .or_else(|| binding.options.color_profile.as_deref())
            .or_else(|| binding.options.profile.as_deref()),
    )
...
fn transform_template_array_looks_like_color_tokens(items: &[Value]) -> bool {
    !items.is_empty()
        && items.iter().all(|item| match item {
            ...
            Value::Object(fields) => {
                fields.get("text").is_some_and(Value::is_string)
                    ...
                    || fields.get("label").is_some_and(Value::is_string)
                    || fields.get("role").is_some_and(Value::is_string)
...
fn transform_template_color_token_from_value(
    ...
    match value {
        ...
        _ => Err(format!(
            "color token at index `{index}` must be an object or string"
        )),
    }
}

fn transform_template_canonical_color_role(role: &str) -> String {
    match role.trim() {
        "diagnostic.fatal" => "diagnostic.error",
        "diff.add" => "diff.added",
        "diff.remove" => "diff.removed",
        "status.muted" => "source.gutter",
        other if transform_template_is_color_role(other) => other,
        _ => "syntax.token",
    }
    .to_owned()
}

fn transform_template_render_terminal_color_token(
    token: &TransformTemplateColorToken,
    profile: &TransformTemplateColorOutputProfile,
) -> String {
    if profile.no_color || !profile.supports_role(&token.role) {
        return token.text.clone();
    }

    let Some(sgr) =
        transform_template_terminal_sgr_for_role(&token.role, profile.terminal_capability)
    else {
        return token.text.clone();
    };

    let styled = format!("\u{1b}[{sgr}m{}\u{1b}[0m", token.text);
    if profile.hyperlinks {
        if let Some(href) = token.href.as_deref() {
            return transform_template_terminal_hyperlink(href, &styled);
        }
    }
    styled
}

fn transform_template_terminal_hyperlink(href: &str, text: &str) -> String {
    let href = href
        .chars()
...
fn transform_template_terminal_sgr_for_role(
    role: &str,
    capability: TransformTemplateTerminalColorCapability,
) -> Option<&'static str> {
    match capability {
        TransformTemplateTerminalColorCapability::None => None,
        TransformTemplateTerminalColorCapability::Ansi16 => match role {
            "diagnostic.error" | "status.error" | "diff.removed" => Some("1;31"),
            "diagnostic.warning" | "status.warning" | "diff.changed" => Some("1;33"),
            "diagnostic.info" | "status.info" => Some("1;36"),
            "diagnostic.hint" | "source.gutter" => Some("2;37"),
            "source.line-number" => Some("2;36"),
        ...
        | TransformTemplateTerminalColorCapability::Auto => match role {
            "diagnostic.error" | "status.error" | "diff.removed" => Some("1;38;5;196"),
            "diagnostic.warning" | "status.warning" | "diff.changed" => Some("1;38;5;214"),
            "diagnostic.info" | "status.info" => Some("38;5;39"),
            "diagnostic.hint" | "source.gutter" => Some("2;38;5;244"),
            "source.line-number" => Some("2;38;5;75"),
        ...
        TransformTemplateTerminalColorCapability::Truecolor => match role {
            "diagnostic.error" | "status.error" | "diff.removed" => Some("1;38;2;220;38;38"),
            "diagnostic.warning" | "status.warning" | "diff.changed" => Some("1;38;2;178;104;24"),
            "diagnostic.info" | "status.info" => Some("38;2;0;110;170"),
            "diagnostic.hint" | "source.gutter" => Some("2;38;2;96;105;120"),
            "source.line-number" => Some("2;38;2;64;128;180"),
...
fn transform_template_render_html_color_token(
    token: &TransformTemplateColorToken,
    profile: &TransformTemplateColorOutputProfile,
) -> String {
    let role = transform_template_encode_html_attribute(&token.role);
    let class_role = transform_template_color_role_class(&token.role);
    let text = transform_template_encode_html_text(&token.text);
    let span = match profile.html_mode {
        TransformTemplateHtmlColorMode::Classes => {
            format!(
                r#"<span class="cem-color cem-color-{class_role}" data-role="{role}">{text}</span>"#
            )
        }
        TransformTemplateHtmlColorMode::InlineStyle => {
            let style = transform_template_html_inline_style_for_role(&token.role);
            format!(r#"<span class="cem-color" data-role="{role}" style="{style}">{text}</span>"#)
        }
        TransformTemplateHtmlColorMode::CssCustomProperties => {
            let style = transform_template_html_css_var_style_for_role(&token.role, &class_role);
            format!(
                r#"<span class="cem-color cem-color-{class_role}" data-role="{role}" style="{style}">{text}</span>"#
            )
        }
    };

    if let Some(href) = token.href.as_deref() {
        let href = transform_template_encode_html_attribute(href);
        format!(r#"<a class="cem-color-link" href="{href}">{span}</a>"#)
    } else {
        span
    }
}

fn transform_template_color_role_class(role: &str) -> String {
    ...
    if class_name.is_empty() {
        "syntax-token".to_owned()
    } else {
        class_name
    }
}

fn transform_template_html_inline_style_for_role(role: &str) -> String {
    let color = transform_template_html_color_for_role(role);
    match role {
        ...
        "diagnostic.hint" | "source.gutter" | "syntax.comment" => {
            format!("color: {color}; font-style: italic")
        }
        "diff.added" => format!("color: {color}; font-weight: 600"),
        "diff.removed" => format!("color: {color}; text-decoration: line-through"),
...
fn transform_template_html_css_var_style_for_role(role: &str, class_role: &str) -> String {
    ...
    match role {
        ...
        "diagnostic.hint" | "source.gutter" | "syntax.comment" => format!(
            "color: var(--cem-color-{class_role}, {fallback}); font-style: italic"
        ),
        "diff.removed" => format!(
            "color: var(--cem-color-{class_role}, {fallback}); text-decoration: line-through"
        ),
        _ => format!("color: var(--cem-color-{class_role}, {fallback})"),
    }
}

fn transform_template_html_color_for_role(role: &str) -> &'static str {
    match role {
        "diagnostic.error" | "status.error" | "diff.removed" => "#b42318",
        "diagnostic.warning" | "status.warning" | "diff.changed" => "#a15c00",
        "diagnostic.info" | "status.info" => "#0969da",
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TransformTemplateCemTreeLayout {
    Inline,
    Block,
}

impl TransformTemplateCemTreeLayout {
    fn as_str(self) -> &'static str {
        match self {
            Self::Inline => "inline",
            Self::Block => "block",
...
fn transform_template_format_cem_tree_object(
    ...
    for field in ["children", "nodes", "slots"] {
        if let Some(value) = fields.remove(field) {
            let child_layout =
                layout.unwrap_or_else(|| transform_template_cem_tree_child_layout(&value));
            fields.insert(
                field.to_owned(),
    ...
    if transform_template_cem_tree_has_children(&fields) {
        let layout = layout.unwrap_or(TransformTemplateCemTreeLayout::Inline);
        fields.entry("formatLayout".to_owned()).or_insert_with(|| {
            transform_template_cem_tree_format_decision(
                "layout",
                "formatter.layout",
                layout.as_str().to_owned(),
                None,
                source_map.as_ref(),
            )
        });
...
fn transform_template_insert_formatter_block_child_layout_nodes(
    nodes: Vec<Value>,
    options: &TransformTemplateEncodeOptions,
    depth: usize,
) -> Vec<Value> {
    let mut formatted = Vec::with_capacity(nodes.len().saturating_mul(3));
    let indent = transform_template_cem_tree_formatter_indent_data(options, depth);
    let line_ending = transform_template_cem_tree_formatter_line_ending_data(options);
    for node in nodes {
        if transform_template_cem_tree_value_is_whitespace(&node) {
            continue;
        }
        let source_map = transform_template_first_cem_tree_source_map(&node);
...
fn transform_template_cem_tree_content_boundary_nodes(
    options: &TransformTemplateEncodeOptions,
    layout: TransformTemplateCemTreeLayout,
    source_map: Option<&SourceMapStack>,
) -> Value {
...
fn transform_template_cem_tree_child_layout_for_fields(
    fields: &serde_json::Map<String, Value>,
) -> Option<TransformTemplateCemTreeLayout> {
    fields
        .get("children")
        .or_else(|| fields.get("nodes"))
        .or_else(|| fields.get("slots"))
        .map(transform_template_cem_tree_child_layout)
}

fn transform_template_cem_tree_child_layout(value: &Value) -> TransformTemplateCemTreeLayout {
    let mut child_count = 0usize;
    let mut all_text = true;
    transform_template_visit_cem_tree_renderable_children(value, &mut |child| {
        child_count += 1;
        if !transform_template_cem_tree_value_is_text(child) {
            all_text = false;
        }
    });

    if child_count > 0 && all_text {
        TransformTemplateCemTreeLayout::Inline
    } else {
        TransformTemplateCemTreeLayout::Block
    }
...
fn transform_template_visit_cem_tree_renderable_children(
    value: &Value,
    ...
    match value {
        Value::Array(items) => {
            for item in items {
                if !transform_template_cem_tree_value_is_whitespace(item) {
                    visit(item);
                }
            }
        }
        other if !transform_template_cem_tree_value_is_whitespace(other) => visit(other),
        _ => {}
    }
}

fn transform_template_cem_tree_value_is_text(value: &Value) -> bool {
    match value {
        Value::String(_) => true,
...
fn transform_template_cem_tree_format_nodes(
    ...
    let source_map = transform_template_first_cem_tree_source_map_in_values(nodes);
    vec![
        transform_template_cem_tree_format_marker(formatter_profile, source_map.as_ref()),
        transform_template_cem_tree_format_decision(
            "line-ending",
            "formatter.line-ending",
            transform_template_cem_tree_formatter_line_ending(options),
            formatter_profile,
            source_map.as_ref(),
        ),
        transform_template_cem_tree_format_decision(
        ...
        transform_template_cem_tree_format_decision(
            "wrapping",
            "formatter.wrapping",
            transform_template_cem_tree_formatter_wrapping(options),
            formatter_profile,
            source_map.as_ref(),
        ),
        transform_template_cem_tree_format_decision(
...
fn transform_template_cem_tree_format_marker(
    formatter_profile: Option<&str>,
    source_map: Option<&SourceMapStack>,
) -> Value {
...
fn transform_template_cem_tree_format_decision(
    name: &str,
    formatter_role: &str,
    value: String,
    formatter_profile: Option<&str>,
    ...
    decision.insert(
        "kind".to_owned(),
        Value::String("format-decision".to_owned()),
    );
...
fn builtin_cem_tree_colorizer(
    ...
    validate_builtin_cem_tree_colorizer_binding(binding)?;
    validate_cem_tree_value(subject).map_err(|message| {
        format!("cem.color-tree expected formatted CEM tree subject: {message}")
    })?;
    let profile = transform_template_binding_color_profile(binding)?;
    let color_profile = binding
        .identity
        .color_profile
        .clone()
        .unwrap_or_else(|| transform_template_canonical_color_profile_selector(&profile));

    let mut tree = subject
        .as_object()
        .ok_or_else(|| "cem.color-tree expected CEM tree object subject".to_owned())?
        .clone();
    tree.insert("colored".to_owned(), Value::Bool(true));
    tree.insert(
...
fn validate_builtin_cem_tree_colorizer_binding(
    binding: &TransformTemplateEncodeBinding,
) -> Result<(), String> {
    if binding.function.kind != TransformTemplateOutputFunctionKind::Color {
        return Err(format!(
            "CEM tree colorizer implementation cannot execute non-color function `{:?}`",
            binding.function.kind
        ));
    }
    if binding.function.name != "cem.color-tree" {
        return Err(format!(
            "CEM tree colorizer implementation `cem.color-tree` cannot execute `{}`",
            binding.function.name
        ));
    }
    if binding.identity.produces != TransformTemplateOutputProducedKind::CemTree {
        return Err(format!(
            "CEM tree colorizer expected `cem-tree` output, got `{}`",
            binding.identity.produces.as_str()
        ));
    }
    if content_type_essence(&binding.identity.target.content_type) != CEM_ML_CONTENT_TYPE {
        return Err(format!(
            "CEM tree colorizer expected content type `{CEM_ML_CONTENT_TYPE}`, got `{}`",
            binding.identity.target.content_type
        ));
    }
    if binding.identity.target.schema != CEM_ML_SCHEMA_URI {
        return Err(format!(
            "CEM tree colorizer expected schema `{CEM_ML_SCHEMA_URI}`, got `{}`",
            binding.identity.target.schema
        ));
    }
    if binding.identity.target.category != "cem-tree" {
        return Err(format!(
            "CEM tree colorizer expected category `cem-tree`, got `{}`",
            binding.identity.target.category
        ));
    }
    Ok(())
...
fn transform_template_apply_cem_tree_color_to_value(
    ...
    match value {
        Value::Array(items) => {
            for item in items {
                transform_template_apply_cem_tree_color_to_value(item, profile, color_profile);
            }
        }
        Value::Object(fields) => {
...
fn transform_template_apply_cem_tree_color_to_node(
    fields: &mut serde_json::Map<String, Value>,
    profile: &TransformTemplateColorOutputProfile,
    color_profile: &str,
) {
    let role = transform_template_cem_tree_node_color_role(fields);
...
fn transform_template_cem_tree_node_color_role(fields: &serde_json::Map<String, Value>) -> String {
...
fn transform_template_cem_tree_color_generated_source_map_value(
    fields: &serde_json::Map<String, Value>,
) -> Option<Value> {
    let source_map = transform_template_cem_tree_node_source_map(fields)?;
    serde_json::to_value(transform_template_source_map_with_transform(
...
fn transform_template_cem_tree_node_source_map(
    fields: &serde_json::Map<String, Value>,
) -> Option<SourceMapStack> {
    fields
        .get("sourceMap")
        ...
        .or_else(|| {
            fields
                .get("outputSpan")
                .and_then(|value| serde_json::from_value::<OutputSpan>(value.clone()).ok())
                .map(|span| span.origin)
        })
}

fn transform_template_source_map_with_transform(
    ...
    source_map.push(SourceMapFrame {
        ...
        span: current
            ...
            .unwrap_or(FrameSpan::Single(ByteRange::new(0, 0))),
        transform,
    });
    source_map
}
...
fn transform_template_merge_cem_tree_color_writer_attribute_nodes(
    ...
    let nodes = merged
        ...
        .map(|(name, (value, source_map))| {
            ...
            node.insert(
                "colorizerRole".to_owned(),
                Value::String("colorizer.writer-attribute".to_owned()),
            );
            node.insert(
                "colorProfile".to_owned(),
                Value::String(color_profile.to_owned()),
...
fn builtin_cem_ql_text_encoder(
    ...
    let text = subject
        .as_str()
        .ok_or_else(|| "cem-ql.text expected string subject".to_owned())?;
...
fn builtin_rnc_text_encoder(
    ...
    let text = subject
        .as_str()
        .ok_or_else(|| "rnc.text expected string subject".to_owned())?;
...
fn builtin_source_text_content_type_matches(
    function_name: &str,
...
pub fn transform_template_format_cem_source_text(
    ...
    transform_template_validate_source_text_formatter_options(
        options,
        formatter_profile,
        "CEM",
        "cem",
...
pub fn transform_template_format_cem_ql_source_text(
    value: &str,
    options: &TransformTemplateEncodeOptions,
    formatter_profile: Option<&str>,
) -> Result<String, String> {
...
pub fn transform_template_format_rnc_source_text(
    value: &str,
    options: &TransformTemplateEncodeOptions,
    formatter_profile: Option<&str>,
) -> Result<String, String> {
...
fn transform_template_validate_source_text_formatter_options(
    options: &TransformTemplateEncodeOptions,
    formatter_profile: Option<&str>,
    syntax_name: &str,
    syntax_prefix: &str,
...
fn transform_template_apply_source_text_line_ending(
    output: String,
...
fn transform_template_writer_cem_tree_artifact_to_text(
    ...
    let (source_map, output_spans) = match artifact.identity.source_map_policy {
        ...
        TransformTemplateSourceMapPolicy::Generated
        | TransformTemplateSourceMapPolicy::Preserve => {
            let output_spans = if rendered.output_spans.is_empty() {
                artifact.output_spans.clone()
            } else {
                rendered.output_spans.clone()
            };
...
fn validate_cem_tree_formatter_metadata(
    ...
    let format_nodes = value
        .get("formatNodes")
        .and_then(Value::as_array)
        .filter(|nodes| !nodes.is_empty())
        .ok_or_else(|| {
            "CEM tree writer requires formatter-owned `formatNodes`; run `cem.format-tree` before the writer"
                .to_owned()
        })?;
    ...
    if !has_decision {
        return Err(
            "CEM tree writer requires formatter decision nodes in `formatNodes`; run `cem.format-tree` before the writer"
                .to_owned(),
        );
    }
    if let Some(conflicting_profile) = format_nodes.iter().find_map(|node| {
        let profile = node.get("formatterProfile").and_then(Value::as_str)?;
        (profile != formatter_profile).then_some(profile)
    }) {
        return Err(format!(
...
impl TransformTemplateCemTreeRenderedText {
    ...
    fn push_mapped(&mut self, text: &str, source_map: Option<&SourceMapStack>) {
        ...
        self.text.push_str(text);
        let len = (self.text.len() as u64).saturating_sub(start);
        if len == 0 {
            return;
        }
        ...
        let mut origin = source_map.clone();
        origin.push(SourceMapFrame {
            source_id: origin
                ...
                .map(|frame| frame.source_id)
                .unwrap_or(SourceId(0)),
            span: FrameSpan::Single(ByteRange::new(start, len as u32)),
            transform: TransformKind::InterpreterRender,
        });
        self.output_spans.push(OutputSpan {
            output_range: ByteRange::new(start, len as u32),
            origin,
        });
    }
}
...
fn transform_template_render_cem_tree_nodes(
    ...
    for (index, node) in nodes.iter().enumerate() {
        ...
        transform_template_render_cem_tree_node(node, syntax, rendered)?;
    }
    Ok(())
}

fn transform_template_render_cem_tree_node(
    node: &Value,
    syntax: TransformTemplateCemTreeWriterSyntax,
    rendered: &mut TransformTemplateCemTreeRenderedText,
) -> Result<(), String> {
    match node {
        Value::String(text) => match syntax {
            TransformTemplateCemTreeWriterSyntax::Cem => {
                rendered.push_unmapped(&transform_template_encode_cem_content_text(text)?);
                Ok(())
            }
            TransformTemplateCemTreeWriterSyntax::Html => {
                rendered.push_unmapped(&transform_template_encode_html_text(text));
                Ok(())
            }
            TransformTemplateCemTreeWriterSyntax::Xml => {
                rendered.push_unmapped(&transform_template_encode_xml_text(text));
                Ok(())
            }
        },
        Value::Object(fields) => {
            transform_template_render_cem_tree_object_node(fields, syntax, rendered)
        }
        other => Err(format!(
...
fn transform_template_render_cem_tree_object_node(
    fields: &serde_json::Map<String, Value>,
    syntax: TransformTemplateCemTreeWriterSyntax,
    rendered: &mut TransformTemplateCemTreeRenderedText,
) -> Result<(), String> {
    ...
    match kind {
        ...
        _ => transform_template_render_cem_tree_element(fields, rendered),
    }
}
    ...
    match kind {
        "text" | "content" | "string" => {
            let text = match syntax {
                TransformTemplateCemTreeWriterSyntax::Html => transform_template_encode_html_text(
                    transform_template_cem_tree_node_data(fields),
                ),
                TransformTemplateCemTreeWriterSyntax::Xml => transform_template_encode_xml_text(
                    transform_template_cem_tree_node_data(fields),
                ),
        ...
        "comment" => {
            let text = format!("<!--{}-->", transform_template_cem_tree_node_data(fields));
            rendered.push_mapped(&text, source_map.as_ref());
            Ok(())
        }
        "cdata" if syntax == TransformTemplateCemTreeWriterSyntax::Xml => {
            let text = format!(
                "<![CDATA[{}]]>",
                transform_template_cem_tree_node_data(fields)
            );
        ...
        "attribute" | "attr" => Ok(()),
        _ => transform_template_render_cem_tree_markup_element(fields, syntax, rendered),
    }
}

fn transform_template_render_cem_tree_markup_element(
    fields: &serde_json::Map<String, Value>,
    syntax: TransformTemplateCemTreeWriterSyntax,
    rendered: &mut TransformTemplateCemTreeRenderedText,
) -> Result<(), String> {
    ...
    for (name, value, source_map) in
        transform_template_cem_tree_markup_attribute_entries_with_source(fields)?
    {
        let text = format!(
            " {}",
...
fn transform_template_render_cem_tree_element(
    fields: &serde_json::Map<String, Value>,
    rendered: &mut TransformTemplateCemTreeRenderedText,
) -> Result<(), String> {
    let raw_name = fields
    ...
    if has_children {
        ...
        if let Some(format_before_close) = fields.get("formatBeforeClose") {
...
fn transform_template_render_cem_tree_children(
    ...
    match children {
        ...
        Value::Object(_) | Value::String(_) => {
            transform_template_render_cem_tree_node(children, syntax, rendered)
        }
        other => Err(format!(
...
fn transform_template_cem_tree_has_children(fields: &serde_json::Map<String, Value>) -> bool {
    match fields
        .get("children")
        .or_else(|| fields.get("nodes"))
        .or_else(|| fields.get("slots"))
    {
        ...
        Some(Value::Object(_)) | Some(Value::String(_)) => true,
        _ => false,
    }
}
...
fn transform_template_cem_tree_markup_attribute_entries_with_source(
    fields: &serde_json::Map<String, Value>,
) -> Result<Vec<(String, Value, Option<SourceMapStack>)>, String> {
    let mut entries =
        transform_template_cem_tree_markup_attribute_entries(fields.get("attributes"))?
            ...
            .map(|(name, value)| {
                (
                    name,
                    value,
                    transform_template_cem_tree_node_source_map(fields),
                )
            })
            .collect::<Vec<_>>();
...
fn transform_template_cem_tree_writer_attribute_node_entries_with_source(
    fields: &serde_json::Map<String, Value>,
) -> Result<Vec<(String, Value, Option<SourceMapStack>)>, String> {
    let Some(writer_attribute_nodes) = fields.get("writerAttributeNodes") else {
        return Ok(Vec::new());
    };
    let items = writer_attribute_nodes.as_array().ok_or_else(|| {
        format!(
            "CEM tree writer expected `writerAttributeNodes` array, got {}",
            json_value_type_name(writer_attribute_nodes)
        )
    })?;
    items
        .iter()
        .map(|item| {
            ...
            let (name, value) = transform_template_cem_tree_markup_attribute_entry(fields)?;
            Ok((
                name,
                value,
                transform_template_cem_tree_node_source_map(fields),
...
fn transform_template_merge_cem_tree_markup_attribute_with_source(
    entries: &mut Vec<(String, Value, Option<SourceMapStack>)>,
    name: String,
    value: Value,
    source_map: Option<SourceMapStack>,
) {
    let Some((_, existing, existing_source_map)) = entries
        .iter_mut()
        .find(|(candidate, _, _)| candidate == &name)
    else {
        entries.push((name, value, source_map));
        return;
    };
    ...
    if source_map.is_some() {
        *existing_source_map = source_map;
    }
}
...
fn append_transform_template_subject_type_hint(candidates: &mut Vec<String>, value: &str) {
    ...
    match normalized.as_str() {
        "cem-tree" | "cem-output-tree" | "formatted-cem-tree" | "colored-cem-tree" => {
            append_transform_template_subject_type_candidate(candidates, "cem-tree");
        }
...
fn append_transform_template_semantic_subject_type_candidates(
    ...
    if let Some(items) = subject.as_array() {
        if transform_template_array_looks_like_color_tokens(items) {
...
fn resolve_encode_subject_expression(
    ...
    let runtime_functions = BTreeMap::new();
    resolve_encode_subject_expression_with_functions(expression, value_bindings, &runtime_functions)
}

fn resolve_encode_subject_expression_with_functions(
    expression: &str,
    value_bindings: &BTreeMap<String, Value>,
    runtime_functions: &BTreeMap<String, CemtRuntimeFunction>,
) -> Option<Value> {
...
mod tests {
    ...
    fn source_text_output_function_descriptor(
        name: &str,
        category: &str,
        content_type: &str,
        schema: &str,
    ...
    fn cem_tree_color_function_descriptor() -> TransformTemplateOutputFunctionDescriptor {
        TransformTemplateOutputFunctionDescriptor {
            kind: TransformTemplateOutputFunctionKind::Color,
            owner: Some("cem".to_owned()),
            ...
            visibility: TransformTemplateModuleVisibility::Public,
            implementation: TransformTemplateOutputFunctionImplementation::Cemt,
            profile: Some("css-custom-properties".to_owned()),
            extends: None,
            capability: None,
            deterministic: true,
            trusted: false,
    ...
    fn cemt_runtime_call_expression_binds_parameters_for_formatter_helpers() {
        ...
        assert_eq!(
            resolve_encode_subject_expression_with_functions(
                r#"call(formatNode, { node: $node, slot: 0 })"#,
                &values,
                &functions,
            ),
            Some(json!({
                ...
                "slot": 0,
                "children": [
                    {"kind": "text", "text": "Ready", "slot": 0},
                    {
                        "kind": "element",
                        "name": "badge",
                        "slot": 1,
                        "children": [{"kind": "text", "text": "New", "slot": 0}]
                    }
                ]
            }))
        );
...
{module |
  ...
  {format-function
    ...
    fn cemt_module_parser_lowers_output_function_declarations() {
        ...
        let colored_cem_tree = response
            .module_options
            .output_functions
            .iter()
            .find(|function| function.name == "cem.color-tree")
            .expect("CEM tree colorizer declaration");
        assert_eq!(
            colored_cem_tree.kind,
            TransformTemplateOutputFunctionKind::Color
        );
        assert_eq!(
            colored_cem_tree.produces,
            TransformTemplateOutputProducedKind::CemTree
        );
        assert_eq!(colored_cem_tree.category, "cem-tree");
        assert_eq!(colored_cem_tree.subject, "cem-tree");
        assert_eq!(
            colored_cem_tree.profile.as_deref(),
...
{module |
  ...
  {encoding-function @name="acme.html-text" @category="html-text" @subject="string" @produces="text" @content-type="text/html" @schema="https://cem.dev/ns/data/html/1" @canonical=true @streamable=true @extends="html.text" |
    ...
    #[test]
    fn builtin_cem_ql_and_rnc_source_text_encoders_preserve_source_with_line_endings() {
        let mut functions = TransformTemplateOutputFunctionRegistry::new();
        functions.register(source_text_output_function_descriptor(
            "cem-ql.text",
            "cem-ql-module",
        ...
        let ql_request = TransformTemplateEncodeBindingRequest::new(
            Value::String("module demo\r\nlet $root := /cem:document\r\n$root".to_owned()),
            TransformTemplateEncodingTarget::new("text/cem-ql", CEM_QL_SCHEMA_URI, "cem-ql-module"),
        )
        .with_options(TransformTemplateEncodeOptions {
            encoder: Some("cem-ql.text".to_owned()),
            line_ending: Some("lf".to_owned()),
            ..TransformTemplateEncodeOptions::default()
        });
        let ql_binding = functions
            .resolve_encode_binding(&ql_request, &BTreeSet::new())
            .expect("CEM-QL source text encoder resolves with authoring alias");
        let ql_encoded = registry
            .encode(&ql_binding, &ql_request.subject)
            .expect("CEM-QL source text encoder runs");
        assert_eq!(
            ql_encoded,
            Value::String("module demo\nlet $root := /cem:document\n$root".to_owned())
        );
        ql_binding
            .artifact_from_value(ql_encoded)
            .validate_insertion(
                &TransformTemplateEncodedArtifactInsertionContext::new(
                    "text/cem-ql",
                    CEM_QL_SCHEMA_URI,
                )
                .with_category("cem-ql-module")
                .with_produces(TransformTemplateOutputProducedKind::Text),
            )
            .expect("CEM-QL source text artifact carries a compatible identity");

        let rnc_request = TransformTemplateEncodeBindingRequest::new(
            Value::String("namespace cem = \"https://cem.dev/ns/cem-ml/1\"\nstart = element cem:document { text }".to_owned()),
            TransformTemplateEncodingTarget::new(
                RELAX_NG_COMPACT_CONTENT_TYPE,
                RELAX_NG_SCHEMA_URI,
                "rnc-document",
        ...
        .with_options(TransformTemplateEncodeOptions {
            encoder: Some("rnc.text".to_owned()),
            line_ending: Some("crlf".to_owned()),
            ..TransformTemplateEncodeOptions::default()
        });
        let rnc_binding = functions
            .resolve_encode_binding(&rnc_request, &BTreeSet::new())
            .expect("RNC source text encoder resolves");
        let rnc_encoded = registry
            .encode(&rnc_binding, &rnc_request.subject)
            .expect("RNC source text encoder runs");
        assert_eq!(
            rnc_encoded,
            Value::String(
                "namespace cem = \"https://cem.dev/ns/cem-ml/1\"\r\nstart = element cem:document { text }"
                    .to_owned()
            )
        );

        let mut wrong_category = rnc_binding.clone();
        wrong_category.identity.target.category = "rnc-pattern".to_owned();
        let category_error = registry
            .encode(&wrong_category, &Value::String("start = text".to_owned()))
            .expect_err("RNC source text encoder refuses mismatched category");
        assert!(category_error.contains("expected category `rnc-document`"));

        let mut unsupported_formatter = ql_binding.clone();
        unsupported_formatter.options.formatter = Some("cem-ql.pretty".to_owned());
        let formatter_error = registry
            .encode(
                &unsupported_formatter,
                &Value::String("module demo".to_owned()),
            )
    ...
    fn format_binding_resolves_cem_tree_formatter_and_builtin_runs() {
        ...
        assert_eq!(
            cem_tree_format_decision(&formatted, "line-ending")["value"],
            "crlf"
        );
        assert_eq!(
            cem_tree_format_decision(&formatted, "indent")["value"],
            "\t"
        );
        assert_eq!(
        ...
        assert_eq!(
            cem_tree_format_decision(&formatted, "wrapping")["value"],
            "88"
        );
        assert_eq!(
            cem_tree_format_decision(&formatted, "whitespace")["value"],
            "formatter-owned"
        );
        assert_eq!(formatted["nodes"][0]["name"], "card");
        assert_eq!(formatted["nodes"][0]["formatLayout"]["value"], "inline");
        assert_eq!(
            formatted["nodes"][0]["formatLayout"]["formatterRole"],
            "formatter.layout"
        );
        assert_eq!(
    ...
    fn format_binding_materializes_formatter_whitespace_in_cem_tree_nodes() {
        ...
        assert_eq!(formatted["nodes"][0]["formatLayout"]["value"], "block");
        assert_eq!(
            formatted["nodes"][0]["formatContentBoundary"][2]["formatterRole"],
            "formatter.line-ending"
        );
        assert_eq!(
            formatted["nodes"][0]["formatContentBoundary"][2]["value"],
            "\r\n"
        );
        assert_eq!(
            formatted["nodes"][0]["formatBeforeClose"]["formatterRole"],
            "formatter.close-indent"
        );
        ...
        assert_eq!(formatted["nodes"][0]["children"][1]["name"], "title");
        assert_eq!(formatted["nodes"][0]["children"][2]["kind"], "whitespace");
        assert_eq!(formatted["nodes"][0]["children"][2]["value"], "\r\n");
        assert_eq!(
            formatted["nodes"][0]["children"][2]["formatterRole"],
            "formatter.line-ending"
        );
        ...
        assert_eq!(formatted["nodes"][0]["children"][4]["name"], "body");
        assert_eq!(formatted["nodes"][0]["children"][5]["kind"], "whitespace");
        assert_eq!(formatted["nodes"][0]["children"][5]["value"], "\r\n");
        assert_eq!(
    ...
    #[test]
    fn format_binding_materializes_block_layout_that_writer_renders() {
        let mut registry = TransformTemplateOutputFunctionRegistry::new();
        registry.register(cem_tree_format_function_descriptor());
        let implementations =
            TransformTemplateEncodeImplementationRegistry::with_builtin_encoders();
    ...
    fn color_binding_transforms_formatted_cem_tree_in_place() {
        ...
        assert_eq!(
            colored["nodes"][0]["writerAttributeNodes"][0]["name"],
            "class"
        );
        assert_eq!(
        ...
        assert_eq!(
            colored["nodes"][0]["writerAttributeNodes"][0]["colorizerOwned"],
            true
        );
        assert_eq!(
            colored["nodes"][0]["writerAttributeNodes"][0]["colorizerRole"],
            "colorizer.writer-attribute"
        );
        assert_eq!(
            colored["nodes"][0]["writerAttributeNodes"][0]["style"]["colorRole"],
            "source.gutter"
        );
        assert_cem_tree_source_map_current_transform(
            &colored["nodes"][0]["writerAttributeNodes"][0]["sourceMap"],
            |transform| {
                matches!(
                    transform,
                    TransformKind::TemplateTransform { function }
        ...
        assert_eq!(
            colored["nodes"][0]["children"][0]["writerAttributeNodes"][0]["value"],
            "cem-color cem-color-syntax-string"
        );
        assert_cem_tree_source_map_current_transform(
            &colored["nodes"][0]["children"][0]["sourceMap"],
            |transform| {
                matches!(
                    transform,
                    TransformKind::TemplateTransform { function }
        ...
        assert_cem_tree_source_map_current_transform(
            &colored["nodes"][0]["children"][0]["children"][0]["sourceMap"],
    ...
    fn color_binding_materializes_cem_tree_output_profile_matrix() {
        ...
        assert!(none["nodes"][0].get("writerAttributeNodes").is_none());
    ...
    fn formatted_cem_tree_color_subject() -> Value {
        let element_source_map =
            serde_json::to_value(source_map_stack(60, 5)).expect("element source map serializes");
        let attribute_source_map =
        ...
        let format_nodes = formatted_cem_tree_format_nodes_value(source_map_stack(54, 0));
        json!({
            "kind": "cem-tree",
            ...
            "nodes": [{
                "kind": "element",
                "name": "card",
                "sourceMap": element_source_map,
                "attributes": [{
                    "kind": "attribute",
                    "name": "tone",
                    "value": "info",
                    "sourceMap": attribute_source_map
                }],
                "children": [{"kind": "text", "value": "Ready", "sourceMap": text_source_map}]
            }]
        })
    }

    fn formatted_cem_tree_format_nodes_value(source_map: SourceMapStack) -> Value {
        let source_map = serde_json::to_value(source_map).expect("source map serializes");
    ...
    fn assert_cem_tree_source_map_current_transform(
        value: &Value,
        predicate: impl FnOnce(&TransformKind) -> bool,
    ) {
        let source_map: SourceMapStack =
            serde_json::from_value(value.clone()).expect("source map deserializes");
        let current = source_map.current().expect("source map has current frame");
        assert!(
            predicate(&current.transform),
            "unexpected current transform: {:?}",
            current.transform
        );
    }
    ...
    #[test]
    fn builtin_colorizers_emit_terminal_and_html_text() {
        let mut functions = TransformTemplateOutputFunctionRegistry::new();
        functions.register(color_output_function_descriptor(
            "terminal.ansi256",
            "terminal-color",
            "text/plain",
            "https://cem.dev/ns/data/text/terminal/1",
        ...
        let implementations =
            TransformTemplateEncodeImplementationRegistry::with_builtin_encoders();
        let tokens = json!([
            {"role": "diagnostic.error", "text": "Broken"},
            {"role": "syntax.string", "text": "\"ok\""}
        ]);
        ...
        let terminal_binding = functions
            .resolve_color_binding(&terminal_request, &BTreeSet::new())
            .expect("terminal color binding resolves")
            .into_encode_binding();
        let terminal = implementations
            .encode(&terminal_binding, &terminal_request.subject)
            .expect("terminal colorizer runs");
        assert_eq!(
            terminal,
            Value::String(
                "\u{1b}[1;38;5;196mBroken\u{1b}[0m\u{1b}[38;5;76m\"ok\"\u{1b}[0m\u{1b}[0m"
                    .to_owned()
            )
        );
        ...
        let html_binding = functions
            .resolve_color_binding(&html_request, &BTreeSet::new())
            .expect("HTML color binding resolves")
            .into_encode_binding();
        let html = implementations
            .encode(&html_binding, &html_request.subject)
            .expect("HTML colorizer runs");
        assert_eq!(
            html,
            Value::String(
    ...
    {body |
      ...
      {$ encode($node.ast, { contentType: "application/cem", schema: "https://cem.dev/ns/cem-ml/1", category: "cem-tree", subjectType: "cem-ast-node" }, { formatter: match($node.ast.kind, { element: $formatterName, default: "cem.format-tree" }), colorizer: match($node.ast.name, { card: $colorizerName, default: "cem.color-tree" }), colorProfile: match(exists($node.ast.children.0), { true: $colorProfile, false: "none" }), canonical: true }) }
        ...
        let evaluated = evaluate_transform_template_encode_expressions(
            ...
            |binding, subject| {
                executed.push(binding.function.name.clone());
                if binding.function.kind == TransformTemplateOutputFunctionKind::Color {
        ...
        assert_eq!(evaluated.encoded.len(), 1);
        assert_eq!(executed, vec!["cem.format-tree", "cem.color-tree"]);
        let evaluated = &evaluated.encoded[0];
        assert_eq!(evaluated.binding.function.name, "cem.color-tree");
        assert_eq!(
...
{module |
  ...
  {template @name="main" @visibility="public" |
    ...
    #[test]
    fn evaluate_encode_expressions_runs_colorizer_bindings() {
        let response = parse_cem_native_template_module_options(
            TransformTemplateModuleParseRequest {
                template: template_input(
                    "templates/runtime-color.cemt",
                    r#"{@doc cem-ml 1}
{module |
  {color-function
      @name="terminal.ansi256"
  ...
  {template @name="main" @visibility="public" |
    {body |
      {$ encode($node.tokens, { contentType: "text/plain", schema: "https://cem.dev/ns/data/text/terminal/1", category: "terminal-color" }, { colorizer: "terminal.ansi256", colorProfile: "ansi-256" }) }
    }
  }
}"#,
                    Some(FormatIdentity {
        ...
        values.insert(
            "node".to_owned(),
            json!({"tokens": [{"role": "diagnostic.error", "text": "Broken"}]}),
        );
        ...
        let evaluated = &evaluated.encoded[0];
        assert_eq!(
            evaluated.binding.function.kind,
            TransformTemplateOutputFunctionKind::Color
        );
        assert_eq!(
            evaluated.artifact.identity.color_profile.as_deref(),
        ...
        assert_eq!(
            ...
            Value::String("\u{1b}[1;38;5;196mBroken\u{1b}[0m\u{1b}[0m".to_owned())
        );
    }

    #[test]
    fn output_color_type_overrides_color_encode_target() {
        let response = parse_cem_native_template_module_options(
            TransformTemplateModuleParseRequest {
                template: template_input(
                    "templates/runtime-output-color.cemt",
                    r#"{@doc cem-ml 1}
{module |
  {color-function
      @name="terminal.ansi256"
  ...
  {template @name="main" @visibility="public" |
    {body |
      {$ encode($node.tokens, { contentType: "text/plain", schema: "https://cem.dev/ns/data/text/terminal/1", category: "terminal-color" }) }
    }
  }
}"#,
                    Some(FormatIdentity {
        ...
        assert_eq!(evaluated.encoded.len(), 1);
        let evaluated = &evaluated.encoded[0];
        assert_eq!(evaluated.binding.function.name, "html.css-vars");
        assert_eq!(evaluated.artifact.identity.target.content_type, "text/html");
        assert_eq!(
            evaluated.artifact.identity.color_profile.as_deref(),
            Some("css-custom-properties")
        );
    ...
    fn encoded_text_artifact_composition_rebases_colored_cem_tree_writer_spans() {
        ...
        let artifact = response.artifact.expect("CEM tree composes to HTML");
        let text = artifact.value.as_str().expect("artifact text");
        let span_start = text.find("<span").expect("span wrapper is written") as u64;
        let ready_start = text.find("Ready").expect("text child is written") as u64;
        let wrapper_span = output_span_at(&artifact.output_spans, span_start);
        assert!(source_map_contains_color_tree_transform(
            &wrapper_span.origin
        ));
        assert!(matches!(
            wrapper_span.origin.current().unwrap().transform,
            TransformKind::InterpreterRender
        ));

        let ready_span = output_span_at(&artifact.output_spans, ready_start);
        assert!(!source_map_contains_color_tree_transform(
            &ready_span.origin
        ));
        assert!(matches!(
            ready_span.origin.current().unwrap().transform,
            TransformKind::InterpreterRender
        ));
        assert!(matches!(
            ready_span.origin.origin().unwrap().transform,
            TransformKind::CemTokenizer
        ));
    }

    fn output_span_at(spans: &[OutputSpan], start: u64) -> &OutputSpan {
        spans
            .iter()
            .find(|span| span.output_range.start == start)
            .unwrap_or_else(|| panic!("missing output span at byte {start}: {spans:?}"))
    }

    fn source_map_contains_color_tree_transform(source_map: &SourceMapStack) -> bool {
    ...
    #[test]
    fn encoded_text_artifact_composition_rejects_cem_tree_without_formatter_metadata() {
        let mut evaluated = evaluated_colored_cem_tree("body");
        evaluated
            .artifact
            .value
        ...
        assert!(response.diagnostics.iter().any(|diagnostic| {
            diagnostic.code == TRANSFORM_TEMPLATE_ENCODED_ARTIFACT_WRITER_ADAPTER_FAILED_CODE
                && diagnostic.node.as_deref() == Some("body")
                && diagnostic.message.contains("formatNodes")
                && diagnostic.message.contains("cem.format-tree")
        }));
    }

...
use crate::schema::registry::{
    content_type_essence, CEM_AST_JSON_PROJECTION_CONTENT_TYPE, CEM_AST_PROJECTION_CONTENT_TYPE,
    CEM_AST_PROJECTION_SCHEMA_URI,
};

#[derive(Debug, Clone, Copy)]
pub struct CemAstProjectionSourceValidationRequest<'a> {
    pub bytes: &'a [u8],
...
pub fn validate_cem_ast_projection_source_bytes(
    request: CemAstProjectionSourceValidationRequest<'_>,
) -> Vec<Diagnostic> {
    match cem_ast_projection_source_kind(&request) {
        CemAstProjectionSourceKind::Binary => validate_cem_ast_projection_binary(&request),
        CemAstProjectionSourceKind::Json => validate_cem_ast_projection_json(&request),
    }
}

fn cem_ast_projection_source_kind(
    request: &CemAstProjectionSourceValidationRequest<'_>,
) -> CemAstProjectionSourceKind {
    let content_type = request.content_type.map(content_type_essence);
    match content_type.as_deref() {
        Some(CEM_AST_JSON_PROJECTION_CONTENT_TYPE) => CemAstProjectionSourceKind::Json,
        Some(CEM_AST_PROJECTION_CONTENT_TYPE) => CemAstProjectionSourceKind::Binary,
        _ if request.bytes.starts_with(b"CEMPROJ\0") => CemAstProjectionSourceKind::Binary,
...
fn validate_cem_ast_projection_binary(
    ...
    match validate_cem_ast_projection_binary_bytes(request.bytes) {
        Ok(()) => Vec::new(),
        Err((code, message)) => vec![cem_ast_projection_diagnostic(request, code, message)],
    }
}
...
fn validate_cem_ast_projection_binary_bytes(bytes: &[u8]) -> Result<(), (&'static str, String)> {
    ...
    if schema != CEM_AST_PROJECTION_SCHEMA_URI {
        return Err((
            "cem.projection.ast.projection_mismatch",
            format!("binary projection schema `{schema}` is not `{CEM_AST_PROJECTION_SCHEMA_URI}`"),
        ));
    }

    let content_type = reader.read_str("content type")?;
    if content_type != CEM_AST_PROJECTION_CONTENT_TYPE {
        return Err((
            "cem.projection.ast.projection_mismatch",
            format!(
                "binary projection content type `{content_type}` is not `{CEM_AST_PROJECTION_CONTENT_TYPE}`"
            ),
        ));
    }
...
impl<'a> ProjectionBinaryReader<'a> {
        ...
        std::str::from_utf8(bytes)
            ...
            .map_err(|error| {
                (
                    ...
                    format!("CEM AST binary projection {field} is not UTF-8: {error}"),
                )
            })
    }
    ...
    fn read_exact(
        ...
        let end = self.offset.checked_add(len).ok_or_else(|| {
            (
                "cem.projection.ast.binary_truncated",
                format!("CEM AST binary projection {field} length overflows input"),
            )
        })?;
        if end > self.bytes.len() {
            return Err((
                "cem.projection.ast.binary_truncated",
                format!("CEM AST binary projection is truncated while reading {field}"),
            ));
        }
...
fn validate_cem_ast_projection_json(
    ...
    let value = match serde_json::from_slice::<serde_json::Value>(request.bytes) {
        Ok(value) => value,
        Err(error) => {
            return vec![cem_ast_projection_diagnostic(
                request,
    ...
    match validate_cem_ast_projection_json_value(&value) {
        Ok(()) => Vec::new(),
        Err(message) => vec![cem_ast_projection_diagnostic(
            request,
...
fn validate_cem_binary_projection_json(
    ...
    if let Some(chunks) = object.get("chunks") {
        validate_cem_ast_projection_json_chunks(chunks)?;
    }
    Ok(())
}

fn validate_cem_ast_projection_json_chunks(chunks: &serde_json::Value) -> Result<(), String> {
    let chunks = chunks
        .as_array()
...
fn validate_cem_tree_json_node(
    ...
    match kind {
        ...
        "element" => {
            ...
            validate_cem_ast_json_attributes(object, path)?;
...
fn validate_cem_ast_json_attributes(
    object: &serde_json::Map<String, serde_json::Value>,
    path: &str,
) -> Result<(), String> {
...
fn cem_ast_projection_diagnostic(
    request: &CemAstProjectionSourceValidationRequest<'_>,
    code: &'static str,
    message: String,
) -> Diagnostic {
...
mod tests {
    ...
    #[test]
    fn cem_ast_projection_validator_accepts_binary_fixture() {
        let diagnostics = validate(
            include_bytes!(
                "../../schema-packages/cem-ast-projection/v1/examples/basic-ast.cem-bin"
            ),
            CEM_AST_PROJECTION_CONTENT_TYPE,
        );
    ...
    #[test]
    fn cem_ast_projection_validator_reports_binary_magic() {
        let diagnostics = validate(b"not-a-cem-projection", CEM_AST_PROJECTION_CONTENT_TYPE);

        assert!(has_code(&diagnostics, "cem.projection.ast.binary_magic"));
    }

    #[test]
    fn cem_ast_projection_validator_accepts_basic_json() {
        let diagnostics = validate(
            include_bytes!(
                "../../schema-packages/cem-ast-projection/v1/examples/basic-ast.ast.json"
            ),
    ...
    #[test]
    fn cem_ast_projection_validator_accepts_nested_json() {
        let diagnostics = validate(
            include_bytes!(
                "../../schema-packages/cem-ast-projection/v1/examples/nested-ast.ast.json"
            ),
            CEM_AST_JSON_PROJECTION_CONTENT_TYPE,
        );
    ...
    #[test]
    fn cem_ast_projection_validator_reports_invalid_kind_json_shape() {
        let diagnostics = validate(
            include_bytes!(
                "../../schema-packages/cem-ast-projection/v1/examples/invalid-kind.ast.json"
            ),
            CEM_AST_JSON_PROJECTION_CONTENT_TYPE,
        );

        assert!(has_code(&diagnostics, "cem.projection.ast.json_shape"));
    }
}

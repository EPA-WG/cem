...
impl JsonDocumentAst {
    ...
    pub fn to_generic_data_ast(&self) -> GenericDataDocumentAst {
        let root = self.root.as_ref().map(json_value_ast_to_generic_data_value);
...
pub fn generic_data_ast_to_json_cemt_subject(ast: &GenericDataDocumentAst) -> Value {
    let mut document = serde_json::Map::new();
    document.insert("kind".to_owned(), json!("json-document"));
    document.insert("contentType".to_owned(), json!(JSON_CONTENT_TYPE));
    document.insert("schema".to_owned(), json!(JSON_VALUE_SCHEMA_URI));
    document.insert("source".to_owned(), ast.source.to_cemt_subject());
    document.insert("encoding".to_owned(), json!("utf-8"));
    document.insert(
        "encodingReport".to_owned(),
        json!({
    ...
    document.insert("parseFacts".to_owned(), Value::Array(Vec::new()));
    document.insert("root".to_owned(), generic_data_ast_json_root(ast));
...
fn json_value_ast_to_generic_data_value(value: &JsonValueAst) -> GenericDataValueAst {
    match value {
        JsonValueAst::Object { range, members } => GenericDataValueAst::Mapping {
            source_range: json_source_range_to_generic_data_range(*range),
            entries: members
                .iter()
                .map(|member| GenericDataMappingEntryAst {
                    index: member.index,
                    key: GenericDataValueAst::String {
                        source_range: json_source_range_to_generic_data_range(member.name_range),
                        value: member.name.clone(),
                        lexeme: Some(member.name_lexeme.clone()),
                        style: None,
                    },
                    value: json_value_ast_to_generic_data_value(&member.value),
                    source_range: json_source_range_to_generic_data_range(member.range),
                })
                .collect(),
        },
        JsonValueAst::Array { range, items } => GenericDataValueAst::Sequence {
            source_range: json_source_range_to_generic_data_range(*range),
            items: items
                .iter()
                .map(json_value_ast_to_generic_data_value)
                .collect(),
        },
        JsonValueAst::String {
            range,
        ...
        JsonValueAst::Number {
            ...
            source_range: json_source_range_to_generic_data_range(*range),
            lexeme: lexeme.clone(),
            number_kind: json_number_kind_to_generic(*number_kind),
        },
        JsonValueAst::Boolean { range, value } => GenericDataValueAst::Boolean {
            source_range: json_source_range_to_generic_data_range(*range),
            value: *value,
        },
        JsonValueAst::Null { range } => GenericDataValueAst::Null {
            source_range: json_source_range_to_generic_data_range(*range),
        },
    }
}

fn json_source_range_to_generic_data_range(range: JsonSourceRange) -> GenericDataSourceRangeAst {
...
fn json_number_kind_to_generic(kind: JsonNumberKind) -> GenericDataNumberKind {
    match kind {
        JsonNumberKind::Integer => GenericDataNumberKind::Integer,
        JsonNumberKind::Decimal => GenericDataNumberKind::Decimal,
        JsonNumberKind::Exponent => GenericDataNumberKind::Exponent,
    }
}

fn generic_data_ast_json_root(ast: &GenericDataDocumentAst) -> Value {
    match ast.documents.as_slice() {
        [] => generic_data_null_json_node(&GenericDataSourceRangeAst::generated()),
        [document] => document
            .root
            .as_ref()
            .map(generic_data_value_to_json_cemt_node)
            .unwrap_or_else(|| generic_data_null_json_node(&document.source_range)),
        documents => {
            let source_range = GenericDataSourceRangeAst::generated();
            json!({
                "kind": "array",
...
fn generic_data_value_to_json_cemt_node(value: &GenericDataValueAst) -> Value {
    match value {
        GenericDataValueAst::Mapping {
            ...
            "members": entries
                .iter()
                .map(generic_data_mapping_entry_to_json_member)
                .collect::<Vec<_>>(),
        }),
        ...
        GenericDataValueAst::String {
            ...
            node.insert("kind".to_owned(), json!("string"));
            node.insert("sourceRange".to_owned(), source_range.to_cemt_subject());
            node.insert("sourceMap".to_owned(), source_range.source_map_subject());
            node.insert("value".to_owned(), json!(value));
            if let Some(lexeme) = lexeme.as_deref() {
                node.insert("lexeme".to_owned(), json!(lexeme));
            }
            Value::Object(node)
        }
        GenericDataValueAst::Number {
            ...
            "sourceRange": source_range.to_cemt_subject(),
            "sourceMap": source_range.source_map_subject(),
            "lexeme": generic_data_number_lexeme_for_json(lexeme),
            "numberKind": number_kind.as_json_number_kind(),
        }),
        ...
        | GenericDataValueAst::Alias { source_range, .. } => {
            generic_data_null_json_node(source_range)
        }
    }
}

fn generic_data_mapping_entry_to_json_member(entry: &GenericDataMappingEntryAst) -> Value {
    let name = generic_data_key_to_json_member_name(&entry.key);
    let name_lexeme = serde_json::to_string(&name).unwrap_or_else(|_| "\"\"".to_owned());
    let name_range = entry.key.source_range();
    json!({
        "index": entry.index,
        "name": name,
        "nameLexeme": name_lexeme,
        "nameSourceRange": name_range.to_cemt_subject(),
        "nameSourceMap": name_range.source_map_subject(),
        "sourceRange": entry.source_range.to_cemt_subject(),
        "sourceMap": entry.source_range.source_map_subject(),
        "value": generic_data_value_to_json_cemt_node(&entry.value),
    })
}

fn generic_data_key_to_json_member_name(key: &GenericDataValueAst) -> String {
    match key {
        GenericDataValueAst::String { value, .. } => value.clone(),
        GenericDataValueAst::Number { lexeme, .. } => generic_data_number_lexeme_for_json(lexeme),
        ...
        GenericDataValueAst::Mapping { .. } | GenericDataValueAst::Sequence { .. } => {
            serde_json::to_string(&generic_data_value_to_json_value(key)).unwrap_or_default()
        }
    }
}

fn generic_data_value_to_json_value(value: &GenericDataValueAst) -> Value {
    match value {
        GenericDataValueAst::Mapping { entries, .. } => {
            let mut object = serde_json::Map::new();
            for entry in entries {
                object.insert(
                    generic_data_key_to_json_member_name(&entry.key),
                    generic_data_value_to_json_value(&entry.value),
                );
            }
            Value::Object(object)
        }
        GenericDataValueAst::Sequence { items, .. } => Value::Array(
            items
                .iter()
                .map(generic_data_value_to_json_value)
                .collect::<Vec<_>>(),
        ),
        GenericDataValueAst::String { value, .. } => Value::String(value.clone()),
        GenericDataValueAst::Number { lexeme, .. } => {
            serde_json::from_str::<Value>(&generic_data_number_lexeme_for_json(lexeme))
                .unwrap_or(Value::Null)
        }
        GenericDataValueAst::Boolean { value, .. } => Value::Bool(*value),
        GenericDataValueAst::Null { .. } | GenericDataValueAst::Alias { .. } => Value::Null,
    }
}

fn generic_data_number_lexeme_for_json(lexeme: &str) -> String {
    if serde_json::from_str::<Value>(lexeme).is_ok_and(|value| value.is_number()) {
...
fn generic_data_null_json_node(source_range: &GenericDataSourceRangeAst) -> Value {
    json!({
        "kind": "null",
        "sourceRange": source_range.to_cemt_subject(),
        "sourceMap": source_range.source_map_subject(),

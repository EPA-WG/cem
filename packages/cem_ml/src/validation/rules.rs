...
mod tests {
    ...
    fn first_element_by_local_name<'a>(
        doc: &'a crate::parser::document::CemDocument,
        name: &str,
    ) -> Option<&'a CemAstNode> {
        doc.iter().find(|node| {
    ...
    fn schema_package_diagnostic_codes_declared_by_schema() -> std::collections::BTreeSet<String> {
        let source =
            crate::schema::package_sources::builtin_schema_package_source("schema-package")
                .expect("schema-package source");
        let doc = parse(source.schema_source);
        let schema = first_element_by_local_name(&doc, "schema").expect("schema root");
        let diagnostics = element_child_ids_by_local_name(&doc, schema, "diagnostics")
            .into_iter()
            .filter_map(|node_id| doc.get(node_id))
            .next()
            .expect("schema-package diagnostics declaration");

        element_child_ids_by_local_name(&doc, diagnostics, "diagnostic")
            .into_iter()
            .filter_map(|node_id| {
                let node = doc.get(node_id)?;
                attr_value(&doc, node, "code").map(str::to_owned)
            })
            .collect()
    }

    fn schema_package_diagnostic_codes_emitted_by_rules() -> std::collections::BTreeSet<String> {
        let source = include_str!("rules.rs")
            .split("\n#[cfg(test)]")
            .next()
            .expect("production rule source");
        let mut rest = source;
        let mut codes = std::collections::BTreeSet::new();

        while let Some(index) = rest.find("diag_at(") {
            rest = &rest[index + "diag_at(".len()..];
    ...
    #[test]
    fn schema_package_diagnostics_emitted_by_rules_are_declared_in_schema() {
        let emitted = schema_package_diagnostic_codes_emitted_by_rules();
        let declared = schema_package_diagnostic_codes_declared_by_schema();
        let missing = emitted.difference(&declared).collect::<Vec<_>>();

        assert!(
            !emitted.is_empty(),
            "expected schema-package diagnostics emitted by validation rules"
        );
        assert!(
            missing.is_empty(),
            "schema-package schema is missing validation rule diagnostic declarations: {missing:?}"
        );
    }

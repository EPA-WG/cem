...
2. **The built-in vocabulary stays primitive.** The bootstrap model provides
   ...
   URI scheme/authority/path-prefix constraints and media-type
   essence/parameter-name allow-lists, without moving package meaning into
   Rust.
...
`@mediaTypeParameters` then narrow those primitives declaratively in the
schema document.

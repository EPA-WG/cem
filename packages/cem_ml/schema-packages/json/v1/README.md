...
Readable formatter profiles default to leading commas, no newline after an
opening object or array delimiter, and aligned nested member-value delimiters.
For example, non-empty object or array values open on the next indentation
column after the member separator. Use `--cemt-formatter-option
leadingComma=false` and `--cemt-formatter-option scopeOpeningNewLine=true` to
request the conventional comma-after-item, newline-after-open layout.

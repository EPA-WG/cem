...
export const outputRoot = resolve(workspaceRoot, 'dist/packages/cem-ml-cli-native-brew-arm64');
export const buildRoot = resolve(outputRoot, 'build');
export const artifactRoot = resolve(outputRoot, 'artifacts');
export const cargoTargetRoot = resolve(workspaceRoot, 'dist/target/cem_ml_cli_native_brew_arm64');
...
export function captureCombined(command, args, options = {}) {
...
export async function acquireSyft() {
    const { version, darwinArm64ArchiveSha256 } = deployment.syft;
    const toolRoot = resolve(workspaceRoot, `dist/tools/syft/${version}/darwin-arm64`);
    ...
    const archiveName = `syft_${version}_darwin_arm64.tar.gz`;
    ...
    if (actual !== darwinArm64ArchiveSha256) {
        throw new Error(`pinned Syft archive digest mismatch: expected ${darwinArm64ArchiveSha256}, got ${actual}`);
    }
...
function assertOutputPath(path) {
    ...
    if (
        ...
        !normalized.startsWith(`${outputRoot}/`) &&
        !normalized.startsWith(`${workspaceRoot}/dist/tools/`)
    ) {

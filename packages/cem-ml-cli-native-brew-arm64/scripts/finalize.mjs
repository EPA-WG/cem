...
if (process.env.CEM_ML_RELEASE_SIGNING !== 'required') {
    throw new Error('native macOS release finalization requires CEM_ML_RELEASE_SIGNING=required');
}
...
if (signing.appleReady !== true) {
    throw new Error('native macOS finalization requires Developer ID signing and accepted notarization');
}
const suppliedAttestation = process.env.CEM_ML_GITHUB_ATTESTATION_BUNDLE;
if (!suppliedAttestation?.trim()) {
    throw new Error('native macOS finalization requires CEM_ML_GITHUB_ATTESTATION_BUNDLE');
}
requireFile(suppliedAttestation, 'GitHub artifact-attestation bundle');
const checksum = requireFile(artifactPath(names.checksum), 'native macOS checksum manifest');
for (const line of readFileSync(checksum, 'utf8').trim().split('\n')) {
    const match = line.match(/^[a-f0-9]{64} {2}([^/\\]+)$/);
    if (!match) throw new Error(`invalid native macOS checksum line: ${line}`);
    run('gh', [
        'attestation',
        'verify',

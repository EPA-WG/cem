...
| Content hover forced colors | `yarn nx run @epa-wg/cem-components:verify-content-hover-forced-colors` | Launches Chromium with forced colors active; proves checkable-chip system fills, native-listbox hover boundary color, selected/checked and focus coexistence, disabled suppression, restoration, and passive wrapper isolation. |
...
| `cem-chip` | Compact label or filter toggle. | Default slot is visible label; `label` can provide a fuller name. Without `checkable`, renders a passive `<span>`. With `checkable`, renders a native toggle `<button>` and uses the presence-only `checked` attribute as its initial state. | palette, content, action, bend, inset, typography | Checkable chips mirror their boolean slice through `aria-pressed`; hover retains checked meaning, while removable chips need a separate named remove action. |

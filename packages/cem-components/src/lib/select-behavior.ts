...
function synchronizeModel(instance: HTMLElement, state: SelectState): void {
    ...
    if (payloadChanged) {
        const normalized = normalizeOptions(snapshot.payload.nodes);
...
function normalizeOptions(nodes: readonly SerializedPayloadNode[]): NormalizeResult {
    ...
    let loose: NormalizedGroup | undefined;

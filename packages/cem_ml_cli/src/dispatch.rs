...
const READ_PURPOSES: [ResolvePurpose; 5] = [
...
fn transform_graph_config_from_args(
    context: &eng::EngineContext,
    args: &cli::TransformArgs,
) -> Result<(TransformGraphConfig, String), CliRequestError> {
    ...
    Ok((response.graph, config_source_uri))
}
...
fn transform_graph_lowering_error(error: TransformGraphRequestError) -> CliRequestError {
    match error {
        ...
        TransformGraphRequestError::Engine(error) => CliRequestError::Engine(error),
    }
}
...
fn collect_html_source_diagnostics(
    ...
    for input in inputs {
        let content_type = input_source_content_type(input);
        diagnostics.extend(cem_ml::validation::html::validate_html_source_bytes(
            cem_ml::validation::html::HtmlSourceValidationRequest {
                bytes: &input.bytes,
                source_uri: &input.uri,
                content_type: content_type.as_deref(),
            },
...
fn collect_svg_source_diagnostics(
    ...
    for input in inputs {
        let content_type = input_source_content_type(input);
        diagnostics.extend(cem_ml::validation::svg::validate_svg_source_bytes(
            cem_ml::validation::svg::SvgSourceValidationRequest {
                bytes: &input.bytes,
                source_uri: &input.uri,
                content_type: content_type.as_deref(),
            },
...
fn collect_mathml_source_diagnostics(
    ...
    for input in inputs {
        let content_type = input_source_content_type(input);
        diagnostics.extend(cem_ml::validation::mathml::validate_mathml_source_bytes(
            cem_ml::validation::mathml::MathMlSourceValidationRequest {
                bytes: &input.bytes,
                source_uri: &input.uri,
                content_type: content_type.as_deref(),
            },
...
pub const REPORT_BASENAME_QUERY: &str = "cem-ml.query.report";
...
fn write_query_report(
    ...
    write_report_files(context, &report, &args.report, REPORT_BASENAME_QUERY)
}
...
fn query_failure(
    ...
    if let Err(error) = write_query_report(context, args, inputs, diagnostics.clone()) {
...
pub fn run_query(args: cli::QueryArgs, s: &mut Streams<'_>) -> Outcome {
    ...
    let input = match engine_input(
        ...
        positional_input_scope(&args.data, &input_scope),
    ) {
        Ok(input) => input,
    ...
    let query_identity = eng::FormatIdentity {
        ...
        namespaces: input.root_scope.namespaces.clone(),
    ...
    let (query_uri, query_bytes) = if let Some(source) = args.query.as_deref() {
        ...
        let path = args
            .query_file
            .as_deref()
            .expect("clap requires one query source");
        match read_source(
            &engine_context,
            path,
            "query URI",
            ResolvePurpose::Query,
            Some(&args.query_content_type),
        ) {
            Ok(read) => (read.uri, read.bytes),
            Err(error) => {
                let _ = writeln!(
                    s.stderr,
                    "cem-ml: cannot read query `{}`: {error}",
                    path.display()
                );
                return Outcome::code(EXIT_IO);
            }
    ...
    if let Err(error) = write_result {
        let _ = writeln!(s.stderr, "cem-ml: query output write failure: {error}");
        Outcome::code(EXIT_IO)
    } else {
        Outcome::ok()
    }
...
pub fn run_fixture_parity(args: cli::FixtureParityArgs, s: &mut Streams<'_>) -> Outcome {
    ...
    template_adapters.register(cem_ml::conversion::DomProjectionParityCemtAdapter);
...
"#,
    ...
    #[test]
    fn validate_xslt_source_uses_xslt_validator() {
        let p = write_fixture(
            "validate-xslt-source.xsl",
            r#"<?xml version="1.0" encoding="UTF-8"?>
...
"#,
    ...
    #[test]
    fn validate_xslt_source_accepts_custom_element_alias() {
        let p = write_fixture(
            "validate-xslt-source-custom-element.xsl",
            r#"<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template match="/">
    <article><xsl:if test="$ready"><button>Continue</button></xsl:if></article>
  </xsl:template>
</xsl:stylesheet>
"#,
        );
    ...
    #[test]
    fn validate_xslt_source_accepts_custom_element_fragment_alias() {
        let p = write_fixture(
            "validate-xslt-source-custom-element-fragment.html",
    ...
    #[test]
    fn validate_xslt_source_reports_missing_namespace() {
        let p = write_fixture(
            "validate-xslt-source-missing-namespace.xsl",
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.namespace_missing"));
    }

    #[test]
    fn validate_xslt_source_reports_missing_version() {
        let p = write_fixture(
            "validate-xslt-source-missing-version.xsl",
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.version_missing"));
    }

    #[test]
    fn validate_xslt_source_reports_malformed_version() {
        let p = write_fixture(
            "validate-xslt-source-malformed-version.xsl",
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.version_malformed"));
    }

    #[test]
    fn validate_xslt_source_reports_unsupported_version() {
        let p = write_fixture(
            "validate-xslt-source-unsupported-version.xsl",
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.unsupported_version"));
    }

    #[test]
    fn validate_xslt_source_reports_external_uri_rejected() {
        let p = write_fixture(
            "validate-xslt-source-external-include.xsl",
            r#"<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.external_uri_rejected"));
    }

    #[test]
    fn validate_xslt_source_reports_missing_entrypoint() {
        let p = write_fixture(
            "validate-xslt-source-missing-entrypoint.xsl",
            r#"<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.entrypoint_missing"));
    }

    #[test]
    fn validate_xslt_source_reports_unsupported_construct_warning() {
        let p = write_fixture(
            "validate-xslt-source-extension-warning.xsl",
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "legacy_xslt.unsupported_construct"));
    }

    #[test]
    fn validate_xslt_source_reports_not_well_formed_xml() {
        let p = write_fixture(
            "validate-xslt-source-not-well-formed.xsl",
            r#"<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template match="/">
    <main>
  </xsl:template>
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag["code"] == "cem.xslt.not_well_formed_xml"));
    }
...
fn observable_inputs(
    ...
    match command {
        ...
        cli::Command::Query(a) => {
            let input_defaults = input_scope_defaults(&a.context);
            let context = context(&a.context);
            let input = engine_input(
                &context,
                &a.data,
                infer_input_format(&a.data),
                positional_input_scope(&a.data, &input_defaults),
            )
            .map_err(CliRequestError::Engine)?;
            Ok((vec![input], context))
        }
...
pub fn dispatch<E: CemMlEngine + ?Sized>(
    ...
    match parsed.command {
        ...
        cli::Command::Query(a) => run_query(a, s),

...
`docs/examples/multi-output-native.expected.json`. A global `--out` is rejected
for multiple configured outputs, and `--artifact-json` remains a single-output
debug JSON sidecar. The checked-in error specs

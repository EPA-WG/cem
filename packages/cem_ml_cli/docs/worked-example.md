...
cem-ml convert \
  ...
  --report packages/cem_ml_cli/dist/cemt-output-pipeline.report.cem \
  --report-json packages/cem_ml_cli/dist/cemt-output-pipeline.report.json
```
...
report remains CEM-native by default at
`packages/cem_ml_cli/dist/cemt-output-pipeline.report.cem`; the explicit JSON
projection at `packages/cem_ml_cli/dist/cemt-output-pipeline.report.json`
contains the same conversion metadata under
`reportAst.convert.outputs[].conversion`.
For this example, the report records the `direct-html-output` pipeline id, the
`direct-cemt-output-pipeline` implementation path, and formatter, colorizer,
and writer stages. Formatter and colorizer stages output CEM trees; only the
writer stage emits the requested target-native HTML bytes. A packaged converter
route such as `cem-dom-projection-to-html-cemt` reports its converter id in the
same `converterId` field. The
...
It also compares the generated CEM and JSON reports with the checked-in
`cemt-output-pipeline.report.expected.{cem,json}` files.
...
primary target-native bytes to its own `dest` / `destination`. The checked-in
`docs/examples/multi-output-native.run.json` example writes two CEM-native
...
Those snapshots include conversion metadata for the CEM-native output and omit
it for the generic JSON-to-YAML data conversion, which does not use a CEMT
formatter/colorizer/writer pipeline.

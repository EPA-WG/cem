...
- [ ] **Wishlist (future — NOT in the immediate release timeline):** expanded runtime support for the
      ...
      exports with artifact source maps write `{destination}.map` sidecars with export/input/destination metadata through the output resolver.
...
- [x] Complete the custom-element XSLT parity scope before expanding the component catalog. The engine now has the first
      ...
      predicates and text/attribute terminals, and `match="@*|node()"` template dispatch in document order, union-arm

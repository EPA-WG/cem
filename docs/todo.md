...
- [ ] **Immediate goal: root-scope run configuration for lib + WASM + CLI.** Design homes:
      ...
      budgets; `convertMs` enforces input/output-scope convert work budgets; `traceMs` and `inspectMs` enforce trace
      and inspect work budgets; and effective `baseUri` values now project relative report input and diagnostic URIs.

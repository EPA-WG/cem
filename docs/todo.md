...
- [ ] Enable schema behavior, including `{diagnostic}` behavior, to be defined
  ...
  - [ ] Add schema-package and CLI examples for every engine-provided algorithm
        ...
        length/prefix/suffix constraints, list item-count constraints, plus path prefix/extension, URI scheme/host/port/authority/path-prefix/path-extension, and media-type
...
- [ ] Implement schema-owned field contracts for every schema-declared field
  ...
  - [ ] Extend the compiled Rust schema contract model. `SchemaDocumentModel`
        ...
        URI/media-type constraints beyond initial allowed scheme/host/port/authority/path-prefix/path-extension
  ...
  - [ ] Extend the generic field-contract evaluator. The first evaluator runs
        ...
        occurrence checks, child-set cardinality, exact child-count and total/selected/distinct child-count checks, URI scheme/host/port/authority/path-prefix/path-extension

...
- [ ] Stabilize browser startup waits under parallel Storybook load, after
  ...
  - [x] After that promotion, investigate renderer shadow/try/hook snapshots and
        ...
        See the [proposal](browser-stabilization-review.tmp.md#accepted-direct-owned-input-context-construction).
  ...
  - [x] Decide whether to construct the owned renderer input context directly:
        ...
        [decision](browser-stabilization-review.tmp.md#accepted-direct-owned-input-context-construction).
        Accepted by the user 2026-09-22: promote direct input construction.
  - [x] Promote approved direct owned input-context construction with
        ...
        Fixture: require default construction to reuse the copied document
        allocation and default rendering to skip transient document copies.
        Completed 2026-09-22: both default regressions fail before promotion and
        pass afterward. All 687 native tests and lint pass. Loaded XML setup
        falls 1.268→0.716 ms (43.5% in this profile), preserving owned values,
        metadata, public bindings and native owners. Rebuilt WASM passes 40
...
- [x] Enforce one-to-one source-loaded Storybook coverage for every authored
    ...
    - [x] Fixture: audit both `demo/xpath-aggregates.html` samples against
        ...
        - [x] Apply the already-approved source-link policy to this page's
          ...
          broader coverage and historical startup investigations remain open.
    - [x] Fixture: audit all six `demo/xpath-functions.html` samples against
          their source-loaded Storybook and independent gallery contracts.
          ...
          for any new behavior decision.
        - [x] Check exact string whitespace/Unicode, case-sensitive predicates,
              numeric quantity boundaries, unqualified XML matching and
              descendant text. Verify empty results, malformed-source recovery,
              editor identity/focus/selection and independent sample state.
        - [x] Check all navigation/fragment targets, diagnostics, desktop/mobile
              containment and complete source scrolling. Apply the existing
              source-link opt-in policy to this page's loading declarations.
        - [x] Run only the focused Storybook, unit and independent browser
              checks unless the audit requires changes to shared modules.
          Completed 2026-09-25: six named Storybook steps and shared independent
          contracts cover exact paired results, XML namespaces/descendant text,
          quantity boundaries, empty matches, invalid-source recovery and
          editor retention. Eight navigation links now use the approved source
          base; five current-page fragment links retain all edited state.
          ...
          source-loaded diagnostic histories. Focused validation passes two
          Storybook tests, three source-contract unit tests, both independent
...
- [ ] Support scoped module maps inside `cem-element` styles through typed CSS
    ...
    - [x] Decide CSS override representation/precedence and managed `@import`
          loading before changing the resolver or scoped-CSS contract.
          Accepted: ordinary resource entries, nearest-scope CSS lookup with
          ancestor restrictions preserved, and retained-AST scoped import loading.
    ...
    - [ ] Resolve CSS import expressions (`@import`), `url(...)` references and
        ...
        - [x] Fixture: validate native import supports clauses; retain declaration
              ...
              bad tokens and invalid declarations before attaching downloaded sheets.

...
- [ ] **Immediate goal: root-scope run configuration for lib + WASM + CLI.** Design homes:
      ...
      targets, unsupported non-scheduler budget hooks, and valid-but-not-yet-enforced module-map fields emit
      deterministic execution diagnostics instead of being silently ignored. Keep this item open until module maps and
      those remaining budget hooks drive their real runtime behavior, not just preservation and diagnostics.

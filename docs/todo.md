...
- [ ] **Immediate goal: root-scope run configuration for lib + WASM + CLI.** Design homes:
      ...
      paths against the config document path. Configured and positional input reads resolve local `file://` URIs, and
      primary output writes resolve local `file://` destinations to filesystem paths. Run-config normalization now

...
| CEM-ML schema | Canonical CEM-native syntax, streaming `SchemaMachine`, RELAX NG structural parity, semantic rules, source-map diagnostics, stable URI manifest, TypeScript/Rust headers. | Purpose-built for CEM documents, not full functional parity with every schema language listed here. | Owns the source of truth; emits RELAX NG XML/compact mirrors and TypeScript/Rust headers directly, treats XSD as a downstream adapter, and covers Schematron-style assertions through its semantic rule registry. |

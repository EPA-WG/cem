...
| `cem:stdlib/strings` | Codepoint iteration, length, slicing, casing, formatting, literal splitting, trim variants, character access, search indices, containment, prefix/suffix checks, whitespace normalization, replacement, translation, and substring helpers. |
...
| `cem:stdlib/strings` | Regex matching/splitting (distinct from Tier A literal `split`), Unicode normalization (`nfc`, `nfd`). |

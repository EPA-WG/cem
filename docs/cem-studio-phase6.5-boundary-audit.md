...
| UI foundation       | The component catalog has 49 public primitives and the pinned Angular Material inventory has 37 entries. The initial frame can reuse covered `cem-tree`, `cem-dialog`, `cem-select`, `cem-stepper`, `cem-tabs`, `cem-tooltip`, and related elements; partial rows such as app bar, table, progress, toast, list, and button remain bounded by their recorded contracts.                | Every Studio composition must classify its needed behavior against the matrix. A partial component is usable only within its proven contract; missing general behavior is completed in `@epa-wg/cem-components` first. |

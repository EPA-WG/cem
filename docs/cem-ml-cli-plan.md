...
7. `cem-ml transform <data> --template <file>`
    ...
    - Current design/API slice:
      ...
      writing export files or source-map sidecars. The crate dependency cycle is avoided by keeping the concrete CEM-QL
      adapter in `cem_ml_transform_cem_ql` rather than in `cem_ml`. Mixed-runtime transform graphs now use per-stage
      runtime policy, with CLI coverage for CEM-native and XSLT stages in the same graph. Quoted XPath string literals in
      lowered XSLT value/param expressions now render as text while scalar variables that represent rewritten CEM-QL
      expressions still splice as expressions.

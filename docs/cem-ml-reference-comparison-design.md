...
| `artifact-function-declared`      | manifest artifact path resolved as lookup `artifact-identity`; manifest `@function-name` as lookup `function-name`; selected CEMT output declaration as `actual` `function-identity` projected through `canonicalName` and artifact identity                                                                                   | `schema:exists` after engine-assisted `schema:cemt-output-function` lookup using a composite key of artifact identity plus function name.                                                |
| `artifact-function-contract`      | manifest artifact target metadata and selected CEMT output declaration metadata as record fields: artifact identity, function name/identity, content type identity, schema identity, category, profile, and subject type when declared                                                                                          | `schema:record-fields-equal`; use compare presence `both-or-none` for optional profile field pairs and `schema:required-valid` state policy for kind/content-type/schema/category fields. |

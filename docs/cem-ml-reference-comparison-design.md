...
| `artifact-function-declared`      | manifest `@function-name` as `actual` `function-name`; manifest artifact path resolved as lookup `artifact-identity`; compiled CEMT declarations as `expected` `function-identity` records projected through `canonicalName` and artifact identity                                                                              | `schema:exists` after engine-assisted `schema:function-identity` lookup using a composite key of artifact identity plus function name.                                                    |
| `artifact-function-contract`      | manifest artifact target metadata and compiled CEMT output function identity/metadata as record fields: artifact identity, function name/identity, content type identity, schema identity, category, profile, and subject type when declared                                                                                    | `schema:record-fields-equal`; use compare presence `both-or-none` for optional profile field pairs and `schema:required-valid` state policy for kind/content-type/schema/category fields. |

...
| Processing boundary | Implemented through Phase 3B | Structured-clone guards, chunked text sources, snapshots, render-plan identity, revisions, resource controls, diff frames, privacy/export behavior, and edge-state primitives have focused unit coverage. The package-private `cem-processing-host-v4` protocol adds native document retention/release and explicit render bindings while logical roots share lazily allocated worker slots. Pool-global job IDs, bounded per-slot queues, FIFO-per-root/round-robin dispatch, preemptive cancel controls, fallback, scope release, and sequence-only traces are executable. | Transferable binary source chunks remain a later transport optimization, not a pool/scheduler gap. |
...
| Browser runtime and resources | Implemented for URI declarations and Phase 1 HTTP | `CemElementRuntime` streams or adapts declaration sources, retains fragment/URL/specifier plus resolver identities, resolves imported-template resources against the imported base URL, and routes canonical CEM-ML through the scope-owned worker host. The main thread owns HTTP resolution/policy, multi-chunk response loading, abort/stale guards, portable lifecycle metadata and final patch application. Native CEM-ML import owns JSON/XML/YAML/CSV decoding and retains shared CEM nodes for worker rendering. Legacy and the other resource primitives retain their established paths. | `module-url`, `local-storage`, and `location-element` worker controls remain outside this bounded HTTP slice. |

...
| Processing boundary | Implemented through Phase 3B | Structured-clone guards, chunked text sources, snapshots, render-plan identity, revisions, resource controls, diff frames, privacy/export behavior, and edge-state primitives have focused unit coverage. The package-private `cem-processing-host-v3` protocol adds native document retention/release and explicit render bindings while logical roots share lazily allocated worker slots. Pool-global job IDs, bounded per-slot queues, FIFO-per-root/round-robin dispatch, preemptive cancel controls, fallback, scope release, and sequence-only traces are executable. | Transferable binary source chunks remain a later transport optimization, not a pool/scheduler gap. |

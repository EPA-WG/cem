...
Status: **parse/assembly and parameter query APIs implemented**
...
- [The registry](../packages/cem_ql/src/stdlib.rs) exposes five URL parse/assembly
  functions and thirteen immutable parameter functions. Native URL cores use the scoped Rust `url` 2.5.8 dependency.
...
remains checklist work. The subsequent parameter integration is recorded below.
...
## Parameter query integration (2026-09-25)

All thirteen parameter functions in the value contract now resolve through the
same default/imported URL module, with precise scalar or entry-stream result
types. Known incompatible types fail static checking; the native adapter checks
exact entry shapes, string types and cardinality at runtime. Zero-argument
construction produces an empty stream. Existing user-function isolation and
source-mapped diagnostic propagation also apply to parameter calls.

Eight native query fixtures cover every operation, typed pair-array bindings,
initializer shapes, duplicate order, optional values, UTF-16 sorting, immutable
bound inputs, exact result types, complete embedding maps and upstream errors.
Pair arrays use typed bindings because CEM-QL does not author square-bracket
array literals. The WASM query fixture exercises the same native operations,
including pair bindings through its explicit JSON control boundary.

The schema package's [implemented URL reference](../packages/cem_ml/schema-packages/cem-ql/v1/url-functions.md)
is generated by `cargo run -p cem-ql --example url_reference` from the registry.
Browser/CLI/SSR integration parity and the existing private-dependency publication
boundary remain open; this registration does not claim full URL conformance.


## CLI module-query completion (2026-09-25)

The CLI executes the shared 30-case matrix through explicit expression/module
identities, including imported aliases, user declarations and ordered setter
warnings. Its bridge preserves native typed input/result ownership and source
maps. Modules require a URI declaration and one final expression; local bindings
and functions plus registered built-in imports are supported. External/plugin
loading is deferred under the [module-query contract](cem-ql-cli-module-query-design.md).
